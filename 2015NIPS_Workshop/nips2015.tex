...
Let $\|v\|_2$ denote its $L_2$-norm, and for any matrix $Z$ let $\|Z\|_2=\max_{v:\|v\|_2=1}{\|Z v\|_2}$ denote the corresponding induced norm. Denote the maximal and minimal singular value of $Z$ by $\sigma_{\max}(Z)$ and  $\sigma_{\min}(Z)$, respectively. Also, let $Z_{:i}$ denote the $i$th column of $Z$.
...
We will use $\sigma_{\min}=\sigma_{\min}(A)$ ($\sigma_{\max}=\sigma_{\max}(A)$) to denote 
the smallest (resp., largest) singular value of $A$.
