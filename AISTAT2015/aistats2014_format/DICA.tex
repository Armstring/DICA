...
Our algorithm is a refined version of the algorithm of \cite{hsu2013learning} (HKICA).  
...
Then HKICA \citep{hsu2013learning} is analyzed in this presentation using a measure of functional dependence.
...
We introduce some preliminaries in Section \ref{sec:Preliminaries}. Section \ref{sec:AnalysisHK} is devoted to the analysis of the HK algorithm. Then in section \ref{sec:DICA} we propose our algorithm and show that it has the 'real' provable guarantee.
...
\end{align*} 
\begin{remark}
In practice, HKICA algorithm may generate complex output. 
...
We try to fill the gap in next section. 
...
As mentioned in the Remark \ref{rmk:xi}, these conditions will be satisfied in the traditional stochastic setting of ICA (for a fixed $\gamma_A$), given large enough sample size. \qed
...
Note that $\gamma_A$ is the minimal gap of the eigenvalues of $M$.
...
Such discontinuity of the eigen-spaces suggests that the dependence on $\gamma_A$ is necessary for HK algorithm. 

Despite the important role that $\gamma_A$ plays in the efficiency of HK algorithm, we have no clue about its behavior. 
...
The idea is inspired by the ideas of \citep{arora2012provable} and \citep{frieze1996learning} using a quasi-whitening procedure, as shown in the next section. \qed
\end{remark}
...
\end{algorithm}
\begin{remark}
The difficult in analyzing $\gamma_A$ of the HK algorithm is due to its dependence on the unknown mixing matrix $A$. 
DICA can be viewed in the tensor perspective. 
Instead of flattening two $4$-dimensional tensors into matrices simply by marginalizing in a random direction, in DICA we marginalize in a special direction of structure $\psi^{\top}A^{-1}$ for a random $\psi$. 
Another interpretation of DICA could be in a  probabilistic perspective, as sampling based on an estimation of the variance matrix. 
In particular, in order for $\{v^{\top}A_1, \ldots, v^{\top}A_d \}$ to be independent for some direction $v$ from a Gaussian distribution $N(0,V)$(so that $\gamma_A$ could be easier to analyze), we would like to have $V = A^{-\top}A^{-1}$. 
Despite the true value of $A^{-\top}A^{-1}$ is not available, we can estimate it up to permutation and scaling (which can still maintain the independence of $\{v^{\top}A_1, \ldots, v^{\top}A_d \}$) by $\left(\nabla^2\widehat{f}(\psi)\right)^{-1}$. Then in DICA, $\widehat{B}^{-\top}\phi$ is a way of sampling such direction $v$.
