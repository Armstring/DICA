...
$\vert \E_{X\sim \nu_t^{(s)}} [(AX)^{\otimes i}] \E_{Y\sim \nu_t^{(\epsilon)}} [Y^{\otimes j}]  - \E_{(X, Y)\sim \nu_t^{(s, \epsilon)}} [(AX)^{\otimes i}\otimes Y^{\otimes j}]  \vert \le L/\sqrt{t}$, where $(s,\epsilon)$ is the function by concatenating $s$ and $\epsilon$ together.  
Note that the above assumptions does not lose any generality to the traditional stochastic setting of the ICA model with Gaussian noise independent to the source signals. 
