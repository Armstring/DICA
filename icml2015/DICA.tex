...
\icmlauthor{Andr\'as Gy\"orgy}{gyorgy@gmail.com}
\icmladdress{Department of Computing Science, University of Alberta,
            Edmonton, AB T6G2E8 Canada}
...
Another practically important case to which our setting extends the traditional setting is the Markov sources.  
\begin{example} \em
\label{ex:markov}
Assume that unknown sources $s_i$'s ($1\le i\le d$) are independent (between different components), stationary and ergodic Markov sources. Our observations are similar to the setting in the previous example. 
...
\begin{example}\em
Let $\mu_1$ to be a Benoulli distribution with probability $0.5$ taking value $0.5$ and probability $0.5$ taking value $-0.5$, and $\mu_2$ to be a distribution with density function $p(x) = \frac{1}{\pi \sqrt{1-x^2}}$ for $-1\le x \le 1$. 
For the demonstration example in Figure \ref{fig:demo}, pick $\mu = \mu_1 \otimes \mu_2$.
It is easy to see that $\mu_1$ is the limit distribution of source 1 and $\mu_2$ is the limit distribution of source 2.
For each component, the difference of the induced distribution and its limit distribution is caused by the remaining tail after previous complete periods, thus in a rate $1/T$.
Therefore, it can be verified that the total variance of $\nu_T$ and $\mu$ is also in a $1/T$ rate. 
Thus, $D_4$ is bounded in a rate of $1/T$, since the monomials in the definition of $D_4$ is upper bounded by $1$.
