...
\label{sec:ExpRes}
We compare the empirical performances of all the ICA algorithms mentioned above, including the recursive algorithms and the modified version of DICA, in this section. 
\subsection{Comparison of $\gamma_A$ and $\gamma_R$}
\label{subsec:comparisonGamma}
We compare the behavior of $\gamma_A$ and that of $\gamma_R$ empirically in this section, by testing the expected values of $1/\gamma_A$ and $1/\gamma_R$ for mixing matrices $A$ with different coherences.
In particular, we artificially construct 4 kinds of matrices:
$A_1 = P$; 
...
In particular, 9 algorithms are tested: 
...
the default FastICA algorithm in the 'ITE' toolbox \citep{szabo12separation} (FICA);
the recursive Fourier PCA algorithm posted by \citet{xiao2014FPCAPackage} (FPCA);
and the randomly guessing algorithm (Random).
...
Then we generate a $6$-dimensional BPSK signal $s$ as follows. Let period $=[\sqrt{2},\sqrt{5},\sqrt{7},\sqrt{11},\sqrt{13},\sqrt{19}]$.
For the $i$th component, we generate a binary sequence $queue$ uniformly.
Our observation of the time step $t$, $s_i(t) = i\times\sin (\text{period}(i)*t)$ if $queue(i) = 0$ and $s_i(t) = -i\times\sin (\text{period}(i)*t)$ otherwise.
...
\end{figure*}
The experimental results suggests that moment methods are more robust to high-coherence mixing matrix and Gaussian noise.
FastICA achieve best performance in the case of low coherence.
As the increase of the coherence of the mixing matrix $A$, its performance decreases quickly and becomes sensitive to noise. 
The Fourier PCA algorithm performs poorly, because its assumption is barely satisfied that $A$ is orthonormal. 
...
However, the recursive idea is not always helpful for the moment methods. For a high coherent $A$, the recursive versions outperform their non-recursive versions. 
...
Recursive algorithms suffers from taking many times of such estimations. 
...
We are expecting that DICA will achieve smaller error for an extreme coherent $A$, since $1/\gamma_A$ will be much larger than $1/\gamma_R$. 
...
This error overwhelms the improvement brought by larger eigenvalue spacings, if sample size is not large enough.
...
We will expect that MDICA can have both less estimations of the fourth moments, meanwhile keep the eigenvalue spacing large (intuitively approximation of Gaussian random variable spacings), thus lead to good performance.
...
If the mixing matrix is orthonormal, then FPCA performs better than other algorithms.
...
In the case of extremely coherent mixing matrix, MDICA performs better. Also the recursive idea is very helpful, given the limit of available sample size.
\section{Conclusion}
