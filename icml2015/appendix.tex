...
Assume we have computed an $m$-dimensional subspace in a recursion of depth $k-1$ whose orthonormal projection matrix is $V^{(k-1)} \in \R^{d\times m}$, such that there exist $m$ columns of $\bar{R}$ (WLOG assume it is $1,\ldots,m$) satisfying
\[
\sin\left(\Theta\left(V^{(k-1)}, \bar{R}_{1:m}\right)\right) \le E_{k-1},
\] 
where $\bar{R}_{1:m}$ is the first $m$ columns of $\bar{R}$ and $E_{k-1}$ is an error upper bound for depth $k-1$ recursion.
Then
\begin{align*}
...
Recall that the diagonal elements of $\Lambda$ are squared Cauchy random variables. The following proposition characterizes the maximal spacing of i.i.d. Cauchy random variables.
\begin{prop}
