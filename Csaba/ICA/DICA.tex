...
\begin{thm}
Given that $\xi \le \min\{1,\kappa_{\min}\} \frac{A^2_{(2,\min)}\sigma_{\min}^2(A)}{d^5 A_{(2,\max)}^2A_{\max}^2}$, 
\[ 
\|M - \widehat{M}\|_2 \le \left( \frac{d^5 A_{(2,\max)}^2A_{\max}^2} {\sigma_{\min}^2(A)\kappa_{\min}}+ 4d^5A_{\max}^2 \sigma_{\max}^6(A)\kappa_{\max}^3A_{(2,\max)}^4\right) \xi.
\]
\end{thm}
\begin{proof}
...
Given that $\xi \le \min\{1,\kappa_{\min}\} \frac{A^2_{(2,\min)}\sigma_{\min}^2(A)}{d^5 A_{(2,\max)}^2A_{\max}^2}$, apply Lemma \ref{lem:Mvariation},
\begin{align*}
