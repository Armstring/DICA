...
Thus, we can rescale $A_i$, such that $|\psi^{\top}A_i| = 1$. 
 To investigate how much each $A_i$ is rescaled, we are interested in the behavior of $\min_i |\psi^{\top}A_i|$.
\begin{lemma}
\label{lem:dmin}
...
Let 
\[
\bar{\xi} =  \frac{d^5A_{(2,\max)}^2A_{\max}^2}{\sigma_{\min}^2(A)\kappa_{\min}A^2_{(2,\min)}} \xi
\] 
and 
\begin{equation}
\label{def:gammaR}
...
$\sigma_{\min}(\nabla^2f(\psi)) \ge \kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)$, and 
$\sigma_{\max}(\nabla^2f(\psi)) \le \kappa_{\max}A^2_{(2,\max)}\sigma_{\max}^2(A)$.
\end{lemma}
\begin{proof}
For any unit vector $v$, $v^{\top}AD_{\psi}KA^{\top}v \ge \kappa_{\min} A^2_{(2,\min)}\|v^{\top}A\|_2^2 \ge \kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)$. Similar calculation for the maximum singular value.
\end{proof}

Lastly, we still need to bound $\|M - \widehat{M}\|_2$.
\begin{lemma}
\label{lem:Mvariation_alg}
Given that $\xi \le \frac12\frac{\kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)}{d^5 A_{(2,\max)}^2A_{\max}^2}$, 
\[ 
...
Given that $\xi \le \frac12 \frac{\kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)}{d^5 A_{(2,\max)}^2A_{\max}^2}$, the condition in Lemma \ref{lem:Mvariation} holds. Then apply Lemma \ref{lem:Mvariation} and \cref{lem:nablavariation}, we have
\begin{align*}
\|M - \widehat{M}\|_2 =\, & \|(\nabla^2 f(\phi))(\nabla^2f(\psi))^{-1} - (\nabla^2 \widehat{f}(\phi))(\nabla^2\widehat{f}(\psi))^{-1} \|_2 \\
\le \, &\frac{2\|\nabla^2 f(\phi)\|_2}{\sigma_{\min}^2(\nabla^2f(\psi))}\|E_2\|_2 + \frac{2}{\sigma_{\min}(\nabla^2f(\psi))}\|E_1\|_2 \\
\le \, & 2\left( \frac{A_{(2,\max)}^2\kappa_{\max}\sigma_{\max}^2(A)}{\kappa^2_{\min}A^4_{(2,\min)}\sigma_{\min}^4(A)} + 
\frac{1}{\kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)}
\right)d^5 A_{(2,\max)}^2A_{\max}^2\xi
\end{align*}

Thus, 
\[ 
\|M - \widehat{M}\|_2 \le \frac{2d^5A_{(2,\max)}^4A_{\max}^2\kappa_{\max}\sigma_{\max}^2(A) + 2d^5A_{(2,\max)}^2A_{\max}^2\kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)}{\kappa^2_{\min}A^4_{(2,\min)}\sigma_{\min}^4(A)} \xi.
\]
\end{proof}
...
We need to introduce three lemmas before we can prove the theorem. Without loss of generality, we assume $\kappa_i>0$ for $1\le i \le d$.
...
Given that $\xi \le \frac{\sigma_{\min}^2(A)\kappa_{\min}A^2_{(2,\min)}}{2d^5A_{(2,\max)}^2A_{\max}^2}$, there exist an orthonormal matrix $R^*$ such that 
\[
\|\widehat{B}^{-1}B - R^*\|_2 \le \bar{\xi}.
\]
\end{lemma}
\begin{proof}
Note that $\|B\|_2 \ge \sigma_{\min}(A)\kappa^{1/2}_{\min}A_{(2,\min)}$. For any unit vector $x$,
\begin{align*}
& x^{\top} B^{-1}\widehat{B}\widehat{B}^{\top}B^{-\top}x - x^{\top}x \\
& \quad  = x^{\top}  B^{-1} ( \widehat{B}\widehat{B}^{\top} -BB^{\top} )B^{-\top}x \\
& \quad  \le \|B^{-\top}x \|_2^2 \|E\|_2 \\
& \quad  \le \frac{d^5A_{(2,\max)}^2A_{\max}^2}{\sigma_{\min}^2(A)\kappa_{\min}A^2_{(2,\min)}} \xi = \bar{\xi}.
\end{align*}
Thus given that $\xi \le \frac{\sigma_{\min}^2(A)\kappa_{\min}A^2_{(2,\min)}}{2d^5A_{(2,\max)}^2A_{\max}^2}$, $\bar{\xi} \le 1/2$. 
Also every singular value of $\widehat{B}^{-1}B$ is bounded between $\frac{1}{\sqrt{1+\bar{\xi}}}$ and $\frac{1}{\sqrt{1-\bar{\xi}}}$, i.e. there exist an orthonormal matrix $R^*$ such that 
\[
...
Moreover, note that $f(R^*B^{-\top}\phi) = \sum_i\kappa_i(\phi^{\top}R^*B^{-1}A)_i^4 = \sum_i \kappa_i(\phi^{\top}R^*RK^{-1/2}D_{\psi}^{-1/2})_i^4=\sum_i\kappa_i^{-1}D_{\psi,i}^{-2}(\phi^{\top}R^*R)_i^4$.
...
Given that $\bar{\xi}\le 1/2$, then for $\phi \in \{\phi_1, \phi_2\}$,
\[
...
for $1\le i, j\le d$,
\begin{itemize}
\item $|\phi^{\top}\widehat{B}^{-1}A_i| \le \|\widehat{B}^{-1}B\|_2\|B^{-1}A_i\|_2\le \frac{1}{\sqrt{1-\bar{\xi}}\kappa_{\min}^{1/2}A_{(2,\min)}}$;
\item $|(\widehat{B}^{-1}A)_{ij}| \le \frac{1}{\sqrt{1-\bar{\xi}}\kappa_{\min}^{1/2}A_{(2,\min)}}$;
\end{itemize}
Thus for $1\le i, j\le d$,
\[
|(G_1(\widehat{B}^{-\top}\phi))_{i,j} - (\widehat{G_1}(\widehat{B}^{-\top}\phi))_{i,j}| \le
\frac{d^4}{(1-\bar{\xi})^2\kappa_{\min}^2A^4_{(2,\min)}}D_4(\nu,\nu_t).
\]
Also note that for $1\le i, j\le d$ and any distribution $\Prob{y}$ of $y$,
\begin{itemize}
\item $|\int(\phi^{\top}\widehat{B}^{-1}y)(\widehat{B}^{-1})_{i:}y d\Prob{y}| \le \frac{dC^2}{(1-\bar{\xi})\kappa_{\min}A^2_{(2,\min)}}$; 
\item $|\int(\phi^{\top}\widehat{B}^{-1}y)^2 d\Prob{y}|\le \frac{dC^2}{(1-\bar{\xi}))\kappa_{\min}A^2_{(2,\min)}}$;
\item $|\int(\widehat{B}^{-1})_{i:}yy^{\top}(\widehat{B}^{-\top})_{:j}d\Prob{y}|\le \frac{dC^2}{(1-\bar{\xi}))\kappa_{\min}A^2_{(2,\min)}}$.
\end{itemize}
Thus, 
\[
|(G_2(\widehat{B}^{-\top}\phi))_{i,j} - (\widehat{G_2}(\widehat{B}^{-\top}\phi))_{i,j}| \le
\frac{2d^3C^2}{(1-\bar{\xi})^2\kappa_{\min}^2A^4_{(2,\min)}}D_2(\nu,\nu_t),
\]
and 
\[
...
\le
\frac{d^4}{(1-\bar{\xi})^2\kappa_{\min}^2A^4_{(2,\min)}}\xi, 
\]
thus, 
\begin{equation}
\label{eq:fBhatfhatBhat}
 \frac{1}{12}\|\nabla^2(f(\widehat{B}^{-\top}\phi)) - \nabla^2(\widehat{f}(\widehat{B}^{-\top}\phi))\|_2 \le 
 \frac{d^5}{(1-\bar{\xi})^2\kappa_{\min}^2A^4_{(2,\min)}}\xi.
\end{equation}
...
\frac{1}{12}\|\nabla^2(f(B^{-\top}\phi)) - \nabla^2(\widehat{f}(\widehat{B}^{-\top}\phi))\|_2
\le 
 \frac{d^5}{(1-\bar{\xi})^2\kappa_{\min}^2A^4_{(2,\min)}}\xi + 2\bar{\xi}.
\]
\end{proof}
