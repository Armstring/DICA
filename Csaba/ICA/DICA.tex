...
However, in a probabilistic setting, the concentration inequality holds in general between the empirical distribution and the true distribution (under some boundness conditions), while in the deterministic setting such property is not guaranteed. 
...
Under the assumption that the given sequence is close to some independent distribution in the measure of '***' distance, we state the result of \cite{arora2012provable} in a deterministic setting, as follows. Denote the maximal singular value of a matrix $A$ by $\sigma_{\max}(A)$. Also, let $A_{(2,\min)} = \min_{i} \|A_i\|_2$, $A_{(2,\max)} = \max_{i} \|A_i\|_2$, and $A_{\max} = \max_{i,j} |A_{i,j}|$.
It is easy to verify that $A_{(2,\min)}(A) \ge \sigma_{\min}(A)$ and $A_{(2,\max)}(A) \le \sigma_{\max}(A)$.

\begin{thm}
There exists a constant $c>0$, such that for any zero mean product probability $\nu^*$, non-singular matrix $A$, and signals $g(t)$, if 
\[
D(\nu_T^{A^{-1}g}, \nu^*) \le c\frac{d_{\min}^{9}\kappa_{\min}^{17/2}\sigma_{\min}^{2}(A)}{d^{39/2}\sigma_{\max}^4(A)d_{max}^7\kappa_{\max}^{15/2}\sum_i\Em{\nu^*}{f_i^2}}
\]
...
\[
\|\hat{A}_i - c_iA_{\pi(i)}\|_2 \le C_A D(\nu_T^{A^{-1}g}, \nu^*),
\]
...
\end{lemma} 
\begin{lemma}
Let $\nabla^2 \widehat{f_A}(u) = \Em{\nu^{A^{-1}g}}{(u^{\top}Ax)^4}  - 3\left(\Em{\nu^{A^{-1}g}}{(u^{\top}Ax)^2}\right)^2$, then
\[
\left|E_{i,j}\right| := \left|\left( \nabla^2 f_A(u) - \nabla^2 \widehat{f_A}(u) \right)_{i,j}\right| \le d^4A_{\max}^2 A_{(2,\max)}^2D_4(\nu^{A^{-1}g},\nu^*).
\]
\end{lemma} 
Let $\nabla^2 f_A(u_0) = BB^{\top}$ and $\nabla^2 \widehat{f_A}(u_0) = \hat{B}\hat{B}^{\top}$, thus $B = AK^{1/2}D_A^{1/2}(u_0)R$ for some orthonormal matrix $R$. Then every singular value of $B^{-1}\hat{B}$ is bounded as follows. For any unit vector $x$,
\begin{align*}
& x^{\top} B^{-1}\hat{B}\hat{B}^{\top}B^{-\top}x - x^{\top}x \\
& \quad  = x^{\top}  B^{-1} ( \hat{B}\hat{B}^{\top} -BB^{\top} )B^{-\top}x \\
...
Thus every singular value of $\hat{B}^{-1}B$ is between $\frac{1}{\sqrt{1+\xi}}$ and $\frac{1}{\sqrt{1-\xi}}$. So there exists a orthonormal matrix $R^*$ such that 
\[
\|\hat{B}^{-1}B - R^*\|_2 \le \xi.
\]
...
\]
Similarly we still use the empirical estimation of this function in the algorithm, 
\[
\left|\hat{f}_{\hat{B}^{-1}A}(u) - f_{R^*B^{-1}A}(u) \right| \le \left|\hat{f}_{\hat{B}^{-1}A}(u) - f_{\hat{B}^{-1}A}(u) \right| + \left|f_{\hat{B}^{-1}A}(u) - f_{R^*B^{-1}A}(u) \right|.
\]

Then for the second term 
\begin{align*}
& \left|f_{\hat{B}^{-1}A}(u) - f_{R^*B^{-1}A}(u) \right| = \left| \sum_i \kappa_i(u^{\top}\hat{B}^{-1}A_i)^4 - \sum_i \kappa_i(u^{\top}R^*B^{-1}A_i)^4\right| \\ 
& \quad = \left| \sum_i \kappa_i (u^{\top}\hat{B}^{-1}A_i - u^{\top}R^*B^{-1}A_i)\left[(u^{\top}\hat{B}^{-1}A_i+u^{\top}R^*B^{-1}A_i)\left((u^{\top}\hat{B}^{-1}A_i)^2+(u^{\top}R^*B^{-1}A_i)^2\right)\right]\right| \\
& \quad \le \sum_i \left|\kappa_i ( u^{\top}\hat{B}^{-1}A_i - u^{\top}R^*B^{-1}A_i) \right| \left| u^{\top}\hat{B}^{-1}A_i+u^{\top}R^*B^{-1}A_i\right| \left|(u^{\top}\hat{B}^{-1}A_i)^2+(u^{\top}R^*B^{-1}A_i)^2 \right|
\end{align*}
Note that $\left| u^{\top} \hat{B}^{-1}A_i\right| \le \| \hat{B}^{-1}A_i\|_2 \le \|\hat{B}^{-1}B\|_2\|B^{-1}A_i\|_2 \le \frac{\sqrt{2}}{\kappa_{\min}^{1/2}d_{\min}^{1/2}}$ and $ \left| u^{\top} R^*B^{-1}A_i\right| \le \frac{1}{\kappa_{\min}^{1/2}d_{\min}^{1/2}}$.
\begin{align*}
& \left|f_{\hat{B}^{-1}A}(u) - f_{B^{-1}A}(u) \right| \le \frac{9}{\kappa_{\min}^{3/2}d_{\min}^{3/2}}\sum_i  \left| \kappa_i( u^{\top}\hat{B}^{-1}A_i - u^{\top}R^*B^{-1}A_i )\right|  \\
& \quad \le \frac{9\sqrt{d}}{\kappa_{\min}^{3/2}d_{\min}^{3/2}}\|u^{\top}\hat{B}^{-1}AK - u^{\top}R^*B^{-1}AK\|_2 \\
& \quad \le \frac{9\sqrt{d}}{\kappa_{\min}^{3/2}d_{\min}^{3/2}}\|\hat{B}^{-1}B - R^*\|_2\|B^{-1}AK\|_2 \\
...
Note that $f_{\hat{B}^{-1}A}(u) = \sum_i \kappa_i (u^{\top}\hat{B}^{-1}A_i)^4 $. Let $\alpha_i = |u^{\top}\hat{B}^{-1}A_i| \le \|\hat{B}^{-1}A_i\|_2 \le \frac{\sqrt{2}}{\kappa_{\min}^{1/2}d_{\min}^{1/2}}$.
Thus, 
\begin{align*}
& \quad \left|\hat{f}_{\hat{B}^{-1}A}(u) - f_{\hat{B}^{-1}A}(u) \right| \\
& \le \left| \int (u^{\top}\hat{B}^{-1}Af)^4\,d\nu^* - \int (u^{\top}\hat{B}^{-1}Af)^4\,d\nu^{A^{-1}g} \right|\\
& \quad  + 3\left| \int (u^{\top}\hat{B}^{-1}Af)^2\,d\nu^* - \int (u^{\top}\hat{B}^{-1}Af)^2\,d\nu^{A^{-1}g} \right| \left| \int (u^{\top}\hat{B}^{-1}Af)^2\,d\nu^* + \int (u^{\top}\hat{B}^{-1}Af)^2\,d\nu^{A^{-1}g} \right| \\
& \le \left| \int (u^{\top}\hat{B}^{-1}Af)^4\,d\nu^* - \int (u^{\top}\hat{B}^{-1}Af)^4\,d\nu^{A^{-1}g} \right|\\
& \quad  + 3\left| \int (u^{\top}\hat{B}^{-1}Af)^2\,d\nu^* - \int (u^{\top}\hat{B}^{-1}Af)^2\,d\nu^{A^{-1}g} \right| \left( \left| \int (u^{\top}\hat{B}^{-1}Af)^2\,d\nu^* - \int (u^{\top}\hat{B}^{-1}Af)^2\,d\nu^{A^{-1}g} \right| + 2 \left| \int (u^{\top}\hat{B}^{-1}Af)^2\,d\nu^*\right| \right)\\
...
Thus, 
\[
\left|\hat{f}_{\hat{B}^{-1}A}(u) - f_{B^{-1}A}(u) \right| \le 
...
Given the upper bound for $\left|\hat{f}_{\hat{B}^{-1}A}(u) - f_{B^{-1}A}(u) \right| $, we now need to pick parameters for the algorithm. To guarantee the interval for $\beta$ is not empty, $D_4(\nu^{A^{-1}g},\nu^*)$ has to be small enough. In general, we need to pick 6 parameters. One of the valid picking schemes for the algorithm is as follows.
\begin{align*}
...
\end{align*}
where $Q$ and $K$ will be specified later.  For the algorithm to work correctly, we have several conditions on these parameters:
\begin{enumerate}[(1)]
...
\right]
\]
where $\Lambda_1 = 800\times90000^2$ and $\Lambda_2 = 1200^2$. By the upper bound condition, this interval is not empty.
...
\end{remark}
In the probabilistic setting since $g$ is generated by $A$ and $\nu^*$, $\beta_0$ decreases to 0 with $D_4(\nu_T^{A^{-1}g}, \nu^*)$ as $n$ goes to infinity, thus one only need to pick $\beta$ small enough for the algorithm. 
However, in the deterministic setting there is no guarantee on the existence of such $\beta$, let alone picking such $\beta$. 
 Assume there exists a oracle measure of sequence independence $\text{Dep}(Y)$ that is easy to compute and satisfies some mild properties,  then we can adapt Arora's algorithm to a deterministic setting, as well as a grid search for valid $\beta$.  
...
\]
 ...
 \[
 \text{Dep}(\nu^{\hat{A}^{-1}g}) \le  \text{Dep}(\nu^{\tilde{A}^{-1}g}) \le  \text{Dep}(\nu^{A^{-1}g})+C_{\text{Dep}}D(\nu^{\tilde{A}^{-1}g}, \nu^{A^{-1}g}) \le C_{\text{Dep}}\left(D(\nu^{\tilde{A}^{-1}g}, \nu^{A^{-1}g})  + D(\nu^{A^{-1}g}, \nu^*) \right)  
 \]

It remains to bound $D(\nu^{\tilde{A}^{-1}g}, \nu^{A^{-1}g})$. We only need to bound for $\|\tilde{A}^{-1}g - A^{-1}g\|_2$. Note that 
\[
\|\tilde{A}^{-1}g_i - A^{-1}g_i\|_2 \le \|\tilde{A}^{-1}- A^{-1}\|_2\|g_i\|_2 \le \|\tilde{A}^{-1}\|_2\|A^{-1}\|_2\|\tilde{A}-A\|_2\|g_i\|_2.
\]
Still need to bound $\|\tilde{A}^{-1}\|_2$ and $\|\tilde{A}-A\|_2$, by $\|\tilde{A}_i - c_iA_{\pi(i)}\|_2 $.
\end{proof}
\begin{remark}
...
\STATE Sample $\phi$ from the unit sphere of $\real^d$, and let $\psi = 1\in \real^d$;
