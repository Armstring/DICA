...
If $\Omega = \natural$, $\AA = 2^\natural$ and $\mu_t$ is the uniform distribution on $\{0,\ldots,t\}$.
...
\mu_{2t}(x) = \begin{cases} 1/2, & x=2; \\
					...
					\end{cases}
\]
Then, $\nu^{(f)}_t$, $\nu^{(g)}_t$ are both the uniform Bernoulli distribution on $\{-1,+1\}$, hence they converge and the limit is the same uniform Bernoulli distribution. 
However, while $\nu^{(f+g)}_{2t+1}$ is the uniform Bernoulli distribution on $\{-2,+2\}$,  $\nu^{(f+g)}_{2t}$ is the degenerate
...
\todoc[inline]{What happens when $\Omega = \natural$ and $\mu_t$ is uniform on $\{0,\ldots, t\}$?
}
...
Let $|S|$ denote the length of the set $S\subset \real$, i.e., $|S| = \sup S - \inf S$.
\begin{cor}
Assume $f:\real\rightarrow\real$ and $g:\real\rightarrow\real$ are periodic functions with respective periods $T_1$ and $T_2$. 
Let $\mu_t$ be the uniform measure on $[0,t]$. 
%$\mu_t((a,b)) = (b-a)/t$ for $0\le a\le b\le t$.
 If $T_1/T_2$ is irrational, then $f$ and $g$ are independent. 
\end{cor}
%\vspace{-0.5cm}
\begin{proof}
 For two Borel sets $A\subset \real$ and $B\subset \real$, let $S_1 =f^{-1}(A)\cap [0,T_1] $ and $ S_2 = g^{-1}(B)\cap [0,T_2]$. It is easy to verify that $\nu^{(f)}(A) = \frac{|S_1|}{T_1}$, and $\nu^{(g)}(B) = \frac{|S_2|}{T_2}$. 
 Note that $\nu^{(f)}_t(A\times B) = \frac{|f^{-1}(A)\cap g^{-1}(B)\cap [0,t] |}{t}$. Now assume that $nT_1 \le t\le (n+1)T_1$ and $mT_2\le t \le (m+1)T_2$. Thus,
 \begin{align*}
 ...
 \begin{align*}
  \lim\limits_{n\rightarrow \infty} \nu^{(f)}_t(A\times B) = & \int_{x\in S_1} \lim\limits_{n\rightarrow \infty} \sum_{u=0}^{n-1} \frac{\ind{uT_1+c \in g^{-1}(B)}}{t} \, dc.
 \end{align*}
...
\begin{thm}
\label{thm:CorofICA}
Let $f_1,\ldots,f_d$ be mutually independent real-valued, ergodic functions and let $f = (f_1,\ldots,f_d)$.Moreover, assume $\nu_{f_i}$, $i=1,\ldots,d$ are non-Gaussian distributions. If
\begin{equation}
\left(
\begin{array}{ccc}
g_1 \\
