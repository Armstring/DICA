...
Let 
 \[ 
 Q= \frac{2d^5A_{(2,\max)}^4A_{\max}^2\kappa_{\max}\sigma_{\max}^2(A) + 2d^5A_{(2,\max)}^2A_{\max}^2\kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)}{\kappa^2_{\min}A^4_{(2,\min)}\sigma_{\min}^4(A)} \xi.
 \] 
...
Thus by lemma \ref{lem:dmin}, with probability at least $1/2\times 1/4 = 1/8$,
\[
\gamma \ge \frac{\sqrt{2\pi}A_{(2,\min)}\min_{i\neq j}\|A_i-A_j\|_2}{\sqrt{ed}(d^3+d^2)}.
\]
\end{proof}
...
Let $B$ be the matrix such that $\nabla^2f(\phi) = BB^{\top}$, thus $B = AK^{1/2}D_\phi^{1/2}R^{\top}$ for some orthonormal matrix $R$, where $D_{\phi} = \text{diag}\left((\phi^{\top}A_1)^2,\cdots, (\phi^{\top}A_d)^2\right)$.
...
\quad where $\widehat{m_p}(\eta) = \frac{1}{t}\sum_{k=1}^{t} (\eta^{\top}g(k))^p$, and $\widehat{f}(\eta) = \frac{1}{12}\big(\widehat{m_4}(\eta) - 3\widehat{m_2}(\eta)^2 \big)$;
\STATE Sample $\phi$ from the unit sphere of $\real^d$;
\STATE Compute $\widehat{B}$ such that $\nabla^2\widehat{f}(\phi) = \widehat{B}\widehat{B}^{\top}$;
\STATE Compute $\widehat{M} = \widehat{B}^{\top}(\nabla^2\widehat{f}(\psi))^{-1}\widehat{B}$;
...
Thus, Theoretical results similar to Theorem \ref{thm:efficiency} will still depend on $\gamma$.
...
\begin{equation}
\label{def:gammaR}
\gamma_R =  \min_{i,j: i\neq j} \left\vert (\frac{\phi_1^{\top}(R^*R)_i}{\phi_2^{\top}(R^*R)_i})^2 - (\frac{\phi_1^{\top}(R^*R)_j}{\phi_2^{\top}(R^*R)_j})^2 \right\vert. 
\end{equation}
Similarly, the performance of the algorithm is guaranteed by the following theorem.
\begin{thm}
...
Given that $\xi \le \frac{\sqrt{2\pi}\kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)}{4d^6 A_{(2,\max)}^2A_{\max}^2}$, 
\[ 
...
So there exists an orthonormal matrix $R$ such that $B =AK^{1/2}D_{\psi}^{1/2}R^{\top}$. 
...
Given two symmetric matrices $X$ and $\widehat{X} = X + E$, where $X = HH^{\top}$ and $\widehat{X} = \widehat{H}\widehat{H}^{\top}$, such that $|X^{-1}\|_2 \|E\|_2 < 1$, then every singular value of $H^{-1}\widehat{H}$ is bounded between $1- \|X^{-1}\|_2 \|E\|_2$ and $1+ \|X^{-1}\|_2 \|E\|_2$, 
and every singular value of $\widehat{H}^{-1}H$ is bounded between $\frac{1}{1 + \|X^{-1}\|_2 \|E\|_2}$ and $\frac{1}{1 - \|X^{-1}\|_2 \|E\|_2}$. 
\end{lemma}
\begin{proof}
...
& \quad \le \|X^{-1}\|_2 \|E\|_2.
\end{align*}
Thus every singular value of $H^{-1}\widehat{H}$ is bounded between $1- \|X^{-1}\|_2 \|E\|_2$ and $1+ \|X^{-1}\|_2 \|E\|_2$, 
and every singular value of $\widehat{H}^{-1}H$ is bounded between $\frac{1}{1 + \|X^{-1}\|_2 \|E\|_2}$ and $\frac{1}{1 - \|X^{-1}\|_2 \|E\|_2}$.
\end{proof}
Applying Lemma \ref{lem:matrixsquareroot}, we can get the stability of $B$, as follows.
\begin{lemma}
\label{lem:BhatinverseB}
Given that $\xi \le \frac{\sqrt{\pi}\kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)}{2\sqrt{2}d^6A_{(2,\max)}^2A_{\max}^2}$, under the event $\Epsi$ there exists an orthonormal matrix $R^*$ such that 
\[
...
Given that $\bar{\xi}\le 1/2$, then under the event $\Epsi$ for $\phi \in \{\phi_1, \phi_2\}$,
\[
\|\nabla^2(f(R^*B^{-\top}\phi)) - \nabla^2(\widehat{f}(\widehat{B}^{-\top}\phi))\|_2
\le \frac{4d^9}{(1-\bar{\xi})^2\pi^2\kappa_{\min}^2A^4_{(2,\min)}}\xi + 2\bar{\xi} 
\le\frac{16d^9}{\pi^2\kappa_{\min}^2A^4_{(2,\min)}}\xi + 2\bar{\xi} .
\]
\end{lemma}
\begin{proof}
...
\begin{align*}
\|T - \widehat{T}\|_2  = & \| H^{\top}T_2^{-1}H - \widehat{H}^{\top}\widehat{T}_2^{-1}\widehat{H}\|_2 \\
 \le & 	\| H^{\top}T_2^{-1}H - H^{\top}\widehat{T}_2^{-1}H\|_2  
 		+ \| H^{\top}\widehat{T}_2^{-1}H - \widehat{H}^{\top}\widehat{T}_2^{-1}\widehat{H}\|_2\\
 \le & 	\|H\|_2^2\|T_2^{-1} - \widehat{T}_2^{-1}\|_2 +
 		 \| H^{\top}\widehat{T}_2^{-1}H - \widehat{H}^{\top}\widehat{T}_2^{-1}H\|_2 +
 		 \| \widehat{H}^{\top}\widehat{T}_2^{-1}H - \widehat{H}^{\top}\widehat{T}_2^{-1}\widehat{H}\|_2 \\
 \le &  \|H\|_2^2\|T_2^{-1} - \widehat{T}_2^{-1}\|_2 + 
 		\|H\|_2^2\|\widehat{T}_2^{-1}\|_2\|I - H^{-1}\widehat{H}\|_2 + 
 		\|H\|_2\|\widehat{H}\|_2\|\widehat{T}_2^{-1}\|_2\|I - H^{-1}\widehat{H}\|_2
\end{align*}
\fi
\[
\|H^{-1}\|_2 = \|K^{-1}D_{\psi}^{-2}\Lambda_1\|_2 \le 
\]
\[
...
Note that by Lemma \ref{lem:Binversenablavariation}, for $i = 1,2$
\[
\|\widehat{T}_i - T_i\|_2 = \|\frac{1}{12}\nabla^2(f(R^*B^{-\top}\phi)) - \frac{1}{12}\nabla^2(\widehat{f}(\widehat{B}^{-\top}\phi))\|_2
\le  \frac{4d^5}{\kappa_{\min}^2A^4_{(2,\min)}}\xi + 2\bar{\xi} = Q.
\]
Also  note that 
$
\sigma_{\min}(T_i) = \frac{\min_i (\phi^{\top}R^*R_i)^2}{\kappa_{\max}A_{(2,\max)}}.
$
By Lemma \ref{lem:dmin}, with probability at least $\frac14$, $\min_i (\phi^{\top}R^*R_i)^2 \ge \frac{\pi}{2d^2}$, thus 
\[
Q\le \frac{\pi}{4d^2\kappa_{\max}A_{(2,\max)}} \le \frac{\min_i (\phi^{\top}R^*R_i)^2}{2\kappa_{\max}A_{(2,\max)}} =\frac12 \sigma_{\min}(T_i).
\]
Then by lemma \ref{lem:Mvariation}, since$\|T_i\|_2 = \|K^{-1}D_{\psi}^{-2}\Lambda_i\|_2 \le \frac{1}{\kappa_{\min}A_{(2,\min)}}$,
\[
\|\widehat{T} - T\|_2 \le \frac{2\|X\|_2}{\sigma_{\min}^2(Y)}\|E_2\|_2 + \frac{2}{\sigma_{\min}(Y)}\|E_1\|_2 \le 
\]
by Lemma \ref{lem:Teigenvectorvariation}, there exists a permutation $\pi$ and constants $\{c_1,\ldots,c_d\}$, such that for $1\le k\le d$
\[
