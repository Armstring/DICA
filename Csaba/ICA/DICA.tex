...
In this case, by Lemma \ref{lem:dmin} $k_i$ is scaled by a factor of $ \frac{1}{A_{(2,\max)}} \le k_i \le \frac{2d}{\sqrt{2\pi}A_{(2,\min)}}$. 
...
Denote the diagonal matrix $\text{diag}(\tilde{\kappa}_1,\cdots,\tilde{\kappa}_d)$ by $\tilde{K}$, where $\tilde{\kappa}_i$ is the scaled $\kappa_i$. \citep{DHsu2012}'s algorithm is also modified for such event. 

For $1\le k\le d$, denote $A_{(t)} = (A_k,A_{ck})$, where $A_{ck}$ is the $(d-1)\times d$ matrix $(A_1,\ldots,A_{k-1},A_{k+1},\ldots,A_d)$, where $A_i$ is the $i$-th column of $A$. For $p\ge 1$, $\eta\in \real^d$, 
let 
\begin{equation}
\label{eq:mpdef}
m_p(\eta) = \E_{x\sim \nu^{(s)}}[ (\eta^\top A x)^p ]
...
Basically both conditions require $D_4(\nu,\nu_t)$ to be small enough.

\begin{remark}
So the efficiency of the algorithm of \citet{DHsu2012} depends on $\min_i\|A_i\|_2$ and $\min_{i\neq j}\|A_i-A_j\|_2$. Is it actually independent to the algorithm? If so, we can define it as the condition number of a ICA problem, which define how difficult the ICA problem is. 
\end{remark}
