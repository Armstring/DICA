...
$f = (f_1,\ldots, f_d)^{\top}$ is a $d$-dimension ergodic function, if and only if $g = A(f_1,\ldots,f_d)^{\top}$ is a $m$-dimension ergodic function for any $m\times d$ matrix.
\end{prop}

\begin{proof}
The converse direction is straightforward by picking $A = I_d$.

Now assume $f= (f_1,\ldots, f_d)^{\top}$ is ergodic, thus $\nu_t^{(f)}: \mathcal{B}_d \rightarrow [0,1]$ converge to $\nu^{(f)}$, where $ \mathcal{B}_d$ is the set of all Borel sets of $\real^d$. 
Denote $\bX_t$(and $\bX$) the random vector with distribution $\nu_t^{(f)}$ (and $\nu^{\top}$ respectively). 
Let $\bY_t = (Y_{t,1},\ldots,Y_{t,m})^{\top} = A\bX_t$ and $ \bY = (Y_1,\ldots,Y_m)^{\top} = A\bX$. 
By definition, $\nu_t^{(g)}$(and $\nu^{(g)}$) is the distribution of $A\bX_t$ (and $A\bX$ respectively). 

To prove that $\bY_t$ weakly converge to $\bY$, by Cram\'{e}r-Wold Theorem \citep{cramer1936some} we only need to prove for any $m$-dimension vector $\alpha = (\alpha_1,\ldots,\alpha_m)$, $\alpha\bY_t$ weakly converge to $\alpha\bY$.
Note that again by Cram\'{e}r-Wold Theorem \citep{cramer1936some},
\[
...
%and $\nu^{(\tilde{f}_{ij})}$ are
is guaranteed by proposition \ref{prop:comp}. 
The definition of independent components is easy to extend to the independence of ergodic functions, by requiring that the joint function of all the functions is ergodic with independent components.
\end{remark}
Do there exist ergodic function that has independent (non-independent) components? 
...
\end{prop}
\begin{proof}
The ergodicity of $f+g$ immediately follows Proposition \ref{prop:comp}, i.e. $\nu^{(f+g)} = \lim\limits_{t\ra \infty} \nu_t^{(f+g)}$ exists.

Denote $(X_{t,1},X_{t,2})^{\top}$(and $(X_1,X_2)^{\top}$) the random vector with distribution $\nu_t^{((f,g)^{\top})}$ (and $\nu^{((f,g)^{\top})}$ respectively). 
Note that for any $t\ge0 $, $\nu_t^{(f+g)}$ is the probability of $X_{t,1}+X_{t,2}$, i.e.  $\nu_t^{(f+g)} = \nu_t^{(f)}* \nu_t^{(g)}$. Thus by Cram\'{e}r-Wold Theorem \citep{cramer1936some}, 
\[
\nu^{(f+g)} = \lim\limits_{t\ra \infty} \nu_t^{(f+g)} = \lim\limits_{t\ra \infty} \nu_t^{(f)}* \nu_t^{(g)} = \nu^{(f)} * \nu^{(g)}.
\]  
\if0
...
If $(g,h)^{\top}$ are of independent components then $\nu_{f_i}$, $i=1,\ldots,d$ are Gaussians.
\end{lemma}
\begin{proof}
