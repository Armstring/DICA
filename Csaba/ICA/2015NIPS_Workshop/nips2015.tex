...
x(t) = As(t)+\epsilon(t),
\end{equation}
We consider the problem of reconstructing $A$ having observed $x$.
...
We measure how well a matrix $\hat{A}$ proposed by an algorithm recovers $A$ by the reconstruction error defined as
\[
d(\hat{A}, A) = \inf_{
		\substack{\pi \in \mathrm{Perm}([d]) \\ c\in \real^d}} 
		\max_{k} || c_k A_{:\pi(k)} - A_{:k} ||_2\,,
\]
where $\mathrm{Perm}([d])$ is the set of all the permutations on the set $[d]$.

%We will denote the $i$th component of $s$ by $s_i$. 
We will use the notation $\sigma_{\min}=\sigma_{\min}(A)$ and
$\sigma_{\max}=\sigma_{\max}(A)$ for the smallest (resp., largest) singular value of $A$.
For a function $u:[T] \to \real^k$, we introduce the empirical distribution $\nu^{(u)}$ defined by
$\nu^{(u)}(B)=\tfrac{1}{T}|\{t \in [T]: u(t) \in B\}|$ for Borel sets $B \subset \real^k$.
For a distribution $\mu$ over the reals and let $\kappa(\mu)$ be the absolute kurtosis of $\mu$: 
...
When the infimum is taken over all product measures with zero mean, the measure is denoted by $D_{k}^\circ$.
...
+M(\nu^{(\eps)}) + M( \nu^{(s)}), \Theta(\mu) \right),
\]
where $\mathcal{C}(\mu)$ and $\Theta(\mu)$ are problem dependent constants, polynomial in $(\sigma_{\max}, 1/\sigma_{\min}, 1/\kappa_{\min}(\mu),1/\delta, d, L)$.
