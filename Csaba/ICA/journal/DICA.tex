...
\begin{enumerate}[(i)]
%\vspace{-3mm}
\item $\| \E_{S_i\sim \nu_t^{(s_i)}} [S_i] \|_F\le g(t)$; % L/\sqrt{t}$;
\item $\| \E_{Y \sim \nu_t^{(\epsilon)}} [Y] \|_F \le g(t)$; %L/\sqrt{t}$;
...
\le g(t). %\frac{L}{\sqrt{t}}, % \le L/\sqrt{t},
\end{align*}
...
\end{remark}
%Note that on the event $\Epsi$, $\|\psi^{\top}R\|_2\le L_u$. 
...
\]
where $\hat{A}$ is the output of the DICA algorithm and $Q$ is polinomial in the usual problem parameters and decays to 0 as $T$ grows. %roughly as $1/T$. 
