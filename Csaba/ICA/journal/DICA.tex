...
Using a quasi-whitening procedure, \citet{arora2012provable} reduced the problem to finding all the local optima of a specific function defined using the forth order cumulant, 
...
%It can be seen that our algorithm, DICA, designed for deterministic signals, can solve the problem better than other algorithms, FastICA \citep{hyvarinen1999fast} and HKICA \citep{hsu2013learning}.
...
\begin{definition}
Let $\Epsi$ denote the following event: For some fixed $\ell_l = \frac{\sqrt{\pi}A_{(2,\min)}}{\sqrt{2}d} \ell$ for $0\le \ell\le 1$, and $ L_u \ge \sqrt{2d}$,
...
To force a orthonormal mixing matrix, we will first compute the square root matrix $B$ from $\nabla^2f(\psi) = AD_{\psi}KA^{\top}$ in the same way proposed in DICA. 
