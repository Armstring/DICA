...
\end{prop}
On the other hand, our setting can also cover some other examples excluded by the traditional setting, such as the example of Figure~\ref{fig:demo} in the Introduction.
\begin{example}
...
Now we are ready to state our main results, which show the existence of polynomial-time algorithms for ICA with estimation error that vanishes at an $O(1/\sqrt{T})$ rate for $T$ samples and is also polynomial in the natural parameters of the problem:
...
\end{thm}
\begin{remark}
(i)Note that the result is polynomial in $1/\delta$ which is weaker than being polynomial in $\log(1/\delta)$.
(ii) We will present two algorithms, DICA (Algorithm~\ref{alg:DICA} and HKICA\_recur (Algorithm~\ref{alg:HKICA_recur}) in Section~\ref{sec:DICA} that 
satisfy the theorem.
\end{remark}
...
\] 
\end{prop}  
Denote $\frac{\poly{L_{\eta}, L, d, \sigma_{\max}(A), C}}{\sqrt{T}}$ by $P(L_{\eta})$. Note that this error caused by noise will decay in a rate of $\sqrt{t}$.
...
\begin{align*}
T & \ge \pol\Big(d, L_u, C, \sigma_{\max}(A),\kappa_{\max}, L,\\
& \quad  1/\ell, 1/\kappa_{\min}, 1/\sigma_{\min}(A), 1/\gamma_A\Big),
\end{align*}
assume that there exist a product measure $\mu$ such that 
\[\xi \le \poly{\gamma_A, \frac{1}{d}, \frac{1}{L_u}, \frac{1}{\sigma_{\max}(A)}, \frac{1}{\kappa_{\max}}, \kappa_{\min}, \sigma_{\min}(A), \ell},\] 
is small enough.
...
\[Q=\poly{d, L_u, \sigma_{\max}(A), \kappa_{\max}, \frac{1}{\kappa_{\min}}, \frac{1}{\sigma_{\min}(A)}, \frac{1}{\ell}}.\]
 \end{thm}
 \if0
\begin{remark}
