...
%& \quad \| \hat{A} - ADP\|_2 \\
%\le & 
\poly{d,\frac{1}{\kappa_{\min}}, \frac{1}{\sigma_{\min}}, \frac{1}{\ell}, L_u, L, C, \sigma_{\max}}(Q^2 + \bar{\xi})
\end{align*}
for some diagonal matrix $D$ and permutation matrix $P$.
\end{thm} 
\begin{remark}
Note that when $T$ is large enough, the term $Q^2$ will be dominated by $\bar{\xi}$, which is the error carried over from quasi-whitening.
