...
For example, assuming that we would like to unmix a 2-dimensional observations that is generated as follows:
\begin{itemize}
...
\begin{example}
Assume that each unknown sources $s_i$ is deterministic and periodic. Our observation $x=As+\eps$ is a linear mixture of $s$ contaminated by an i.i.d Gaussian noise for each time step, where $A$ is a non-singular matrix and $\eps\sim {\cN}(0,\Sigma)$ is Gaussian noise.   
...
\end{algorithm}
\begin{remark}
Note that the eigen-values of $\hat{M}$ in DICA\_Mod are $\frac{(\phi^{\top}R_i)^2}{(\psi^{\top}A_i)^4\kappa_i}$ for $1\le i\le d$. 
