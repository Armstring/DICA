...
It is important to note that, under our assumptions, the noise $\epsilon$ has limited effect in the estimation procedure, as shown by the following result:
\begin{prop}
\label{prop:denoise} Suppose Assumptions~\ref{ass:gauss} and~\ref{ass:independence} hold. Then,
for any vector $\eta$ satisfying $\|\eta\|_2\le L_{\eta}$,
\[
\| \nabla^2f_{\nu_T^{(x)}}(\eta) - \nabla^2f_{\nu_T^{(As)}}(\eta) \|_2 \le \frac{\poly{L_{\eta}, L, d, \sigma_{\max}(A), C}}{\sqrt{T}}. 
\] 
\end{prop}  
Denote $\frac{\poly{L_{\eta}, L, d, \sigma_{\max}(A), C}}{\sqrt{T}}$
by $P(L_{\eta})$. Note that this error caused by the noise decays at a rate of $\sqrt{T}$.
...
The idea is inspired by \citet{arora2012provable} and \citet{frieze1996learning} using a quasi-whitening procedure, as shown in the next section. 

\section{A Refined HKICA Algorithm}
\label{sec:DICA}
Recall that $\hat{G}$ is defined in Equation \eqref{eq:G}.
Our new algorithm, named as Deterministic ICA (DICA), is shown in Algorithm \ref{alg:DICA}. 
\begin{algorithm}
...
Let $Z_i = \frac{\phi_1^{\top}R_i}{\phi_2^{\top}R_i}$. Therefore, $Z_i$, $1\le i\le d$ are $d$ independent Cauchy$(0,1)$ random variables. We would need a high probability lower bound for $\gamma_R$
...
\begin{align*}
& \quad \| \hat{A} - ADP\|_2 \\
\le & \poly{d.\frac{1}{\kappa_{\min}}, \frac{1}{\sigma_{\min}(A)}, \frac{1}{\ell}, L_u, L, C, \sigma_{\max}(A)}(Q^2 + \bar{\xi}).
\end{align*}
...
\label{sec:ExpRes}
We compare the empirical performances of all the ICA algorithms mentioned above, including the recursive algorithms and the modified version of DICA (DICA\_Mod), in this section. 
\subsection{Comparison of $\gamma_A$ and $\gamma_R$}
\label{subsec:comparisonGamma}
We compare its behavior and that of $\gamma_R$ empirically. 
...
\subsection{Settings}
In particular, 13 algorithms are tested: 
\begin{itemize}
\vspace{-3mm}
\item Re-sampling version of  HKICA (HKICA\_Re), and its recursive version (HKICA\_Re\_recur);
\item Symmetrizing version of  HKICA  (HKICA\_Symm), and its recursive version (HKICA\_Symm\_recur);
\item Re-sampling version of DICA  (DICA\_Re), and its recursive version (DICA\_Re\_recur);
\item Symmetrizing version of  DICA  (DICA\_Symm), and its recursive version (DICA\_Symm\_recur);
\item Modified version of  DICA  (DICA\_Mod), and its recursive version (DICA\_Mod\_recur);
\item Default FastICA algorithm in the 'ITE' toolbox \citep{szabo12separation} (FICA); 
\item Recursive Fourier PCA algorithm posted by \citet{xiao2014FPCAPackage} (FPCA);
...
We include the Fourier PCA algorithm as another baseline, with a moderately tuned parameter 1. 
The last algorithm is returning a randomly guessing matrix, serving as another baseline.
...
\begin{figure*}[pt]
\label{fig:Error}
	\includegraphics[width =0.5\linewidth]{errorR}
	\includegraphics[width =0.5\linewidth]{error1}
	\includegraphics[width =0.5\linewidth]{error2}
	\includegraphics[width =0.5\linewidth]{error3}
	\includegraphics[width =0.5\linewidth]{error4}
\caption{Reconstruction Error}
\end{figure*}
The experimental results suggests that moment methods are more robust to high-coherence mixing matrix $A$ and Gaussian noise.
FastICA achieve best performance in the case of low coherence.
As the increase of the coherence of $A$, its performance decreases quickly and becomes sensitive to noise. The Fourier PCA algorithm performs poorly, because its assumption is barely satisfied that $A$ is orthonormal. 
This is also confirmed in the result of orthonormal mixing matrix. 
Due to the time limit, we haven't investigate the performance of Fourier PCA with a quasi-whitening procedure.

However, the recursive version is not always helpful for the moment methods. For the high coherent $A$, the recursive versions outperforms their non-recursive version. 
Note that in this case, $A$ is close to singular (smaller minimal singular value), and thus requires a better sample complexity.
...
Recursive algorithms suffers from much more times of such estimations. 
...
We are expecting that DICA\_Re will achieve smaller error in the case of extreme coherent $A$, since $1/\gamma_A$ will be much larger than $1/\gamma_R$. 
However, the experimental results indicate the opposite. 
Note that high coherence implies small minimal singular value.
In this case, the estimation error of $M$ in DICA could be much larger than that in HKICA, because of the fourth degree of the inverse of a high coherent matrix.
Also DICA requires estimation of more fourth moments. These errors, given that we don't have enough samples, overwhelms the improvement brought by larger eigenvalue spacings.
Due to the time limit, we haven't confirm this argument by other experiments yet.

On the other hand, recall the discussion in Remark \ref{rmk:DICA_Mod}. 
We will expect that DICA\_mod can have both less estimations of the fourth moments, meanwhile keep the eigenvalue spacing large (intuitively approximation of Gaussian random variable spacings), thus lead to good performance.
This is confirmed by the experimental results, in both non-recursive and recursive versions.

In summary, the results suggest these moment methods are comparable to each other in practice,
while FastICA  is better for the case of low coherent $A$ or mild coherent $A$ with low noise.
...
In the case of extremely coherent mixing matrix, DICA\_Mod performs better. Also the recursive idea is very helpful, given the limit of available sample size.
\section{Conclusion}
