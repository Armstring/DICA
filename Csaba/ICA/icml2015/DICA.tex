...
One possible example could be as follows.
\begin{example}
\label{eg:1}
Assuming that we would like to unmix a 2-dimensional observations that is generated as follows:
\begin{itemize}
\item for $0\le t\le 15$, $s_1(t) = \sin(t)$; $s_2(t) = \cos(t\pi)$;  
\item mixing matrix $A = \left( 
\begin{tabular}{cc}
1 & -2\\
2.6 & 1.3
\end{tabular}
\right) $;
\item observations $x = As$.
\end{itemize}
Here our observations are not i.i.d. since the sources are deterministic. The result of this example is shown in Figure \ref{fig:demo}.
\begin{figure}[pt]
\label{fig:demo}
\centering
	\includegraphics[width = \linewidth]{demo}
\caption{Example of deterministic sources: The first black curves are deterministic sources; The red curves are our observations; the blue signals are recovered signals from our ICA algorithm (DICA).}
\end{figure}
\end{example} 
...
Given a $4$-dimensional tensor $T$, we denote the matrix $M$ by $T(\eta,\eta,\cdot , \cdot)$ that is generated by marginalizing the first two coordinates of $T$ on the direction $\eta$, i.e
