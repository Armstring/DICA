...
& \quad \times \left(d^5 A_{(2,\max)}^2A_{\max}^2\xi + P\right)
 ...
 \[
  ...
  \frac{1}{\gamma_A} Q(\xi + P)
\]
where $\widehat{A}$ is the output of the HKICA algorithm, and $Q$ is a polynomial in $\{C, L, d, L_u, \sigma_{\max}(A), 1/\sigma_{\min}(A), \kappa_{\max}, 1/\kappa_{\min}, 1/\ell\}$.
 \end{thm}
 \if0
\begin{remark}
