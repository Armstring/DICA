...
\OUTPUT An estimation of the mixing matrix $A$. 
\STATE Sample $\psi$ uniformly from the unit sphere of $\real^d$;
\STATE Evaluate $\nabla^2\widehat{f}(\psi)$, \\
...
The following lemma shows that with high probability, $\|\phi_1\|_2$ and $\|\phi_2\|_2$ are bounded.
\begin{lemma}
\label{lem:lengthofphi}
For $L_1 \le L_2$, with probability $\left(\exp(-1/2L_1^2)-\exp(-1/2L_2^2)\right)^2$, $L_1 \le\|\phi_1\|_2 \le L_2$ and $L_1\le\|\phi_2\|_2 \le L_2$. 
\end{lemma} 
Thus, picking $L_1 =1$ and $L_1 =1$, we have $ 1\le \|\phi_1\|_2\le 2$ and $ 1\le \|\phi_2\|_2\le 2$ with probability $\left(\exp(-1/2) - \exp(-2)\right)^2 > 0.2$.
Denote this event by $\Ephi$.
...
& \widehat{\xi} = \frac{12d^7A^2_{\max}}{\pi\kappa_{\min}A^2_{(2,\min)}}\xi + \frac{4\sqrt{6}\sigma_{\max}^2(A)}{A^2_{(2,\min)}}\bar{\xi},
\end{align*} 
and 
\begin{equation}
\label{def:gammaR}
...
Let 
 ...
 \frac{128d^6A^6_{(2,\max)}}{\pi^3 A^6_{(2,\min)}}\widehat{\xi}.
 \] 
 Assume the following conditions hold:
 \begin{enumerate}
 ...
 (so $\bar{\xi} \le 1/3$);
 \item $\widehat{\xi} \le \frac{\pi A^2_{(2,\min)}}{4d^2A^2_{(2,\max)}}$.
 \end{enumerate}
Then on the event $\Epsi \cap\Ephi\cap \Ephione \cap \Ephitwo$, there exists a permutation $\pi$ and constants $\{c_1,\ldots,c_d\}$, such that for $1\le k\le d$,
\[
...
\end{thm}
\begin{remark}
The 4th condition in fact can be implied by the 5th condition.
\end{remark}

\subsection{Behavior of $\gamma_R$}
\label{subsec:gammaR}
Similarly, the result of Theorem \ref{thm:Modefficiency} essentially depends on $\gamma_R$. 
However, we can calculate a constant-probability lower bound for it, which makes Theorem \ref{thm:Modefficiency} more meaningful. 
