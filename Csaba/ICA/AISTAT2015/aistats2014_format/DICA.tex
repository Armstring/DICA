...
Clearly, $\sigma_{\max}(A) \ge A_{(2,\max)} \ge A_{\max}$, and $\sigma_{\min}(A) \le A_{(2,\min)}$. Denote the vector with all the entries being 1 by a bold symbol $\boldsymbol{1}$.
...
\vspace{-3mm}
\begin{align*}
&\widehat{ G_1}(\eta) = \frac1T\sum_{t=1}^{T} \big(\eta^{\top}x(t)\big)^2x(t)x(t)^{\top}; \\
& \widehat{G_2}(\eta) = \frac{1}{T^2}\sum_{t=1}^{T} \big(\eta^{\top}x(t)\big)^2 \sum_{t=1}^{T}x(t)x(t)^{\top}; \\
& \widehat{G_3}(\eta) = \frac{1}{T^2}\Big(\sum_{t=1}^{T} \big(\eta^{\top}x(t)\big)x(t)\Big) \Big(\sum_{t=1}^{T} \big(\eta^{\top}x(t)\big)x(t)\Big)^{\top}.
\end{align*} 
...
\]
where $\widehat{A}$ is the output of the HKICA algorithm.
 \end{thm}
\begin{remark}
...
A delicate analysis carried by \citet{goyal2014fourier} for a similar spacing problem is heavily based on the boundedness of the characteristic function, which isn't applicable here neither. 
...
\end{itemize}
\vspace{-2mm}
Another orthonormal matrix R is generated by computing the left column space of a non-singular random matrix $N$ (from standard normal distribution).  
...
We then investigate the performances of different ICA algorithms in a simulation setting, with mixing matrices of different coherences. In particular, 6 algorithms are tested: 
\begin{itemize}
\vspace{-3mm}
...
\caption{Reconstruction Error for different noise ratios; Type of mixing matrix: A2.}
\end{figure}
...
On the other hand, we propose the following ICA algorithm trying to relieve the estimation error problem of DICA, but still keeps large gap of the eigen-values in the eigen-decomposition. 
\begin{algorithm}
