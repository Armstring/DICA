...
Denote the vector with all the entries being 1 by a bold symbol $\boldsymbol{1}$. 
$v \le C$ for some vector $v$ and quantity $C$ means all the entries of $v$ are less that $C$, and $\vert v \vert$ means apply the absolute value operation to all the entries of $v$. 
The outer product of vectors $v$ and $u$ is denoted by $v\otimes u$. 
$u^{\otimes k}$ is used as a notion of the outer product of $k$ $v$'s, $v\otimes v\otimes v \ldots \otimes v$, which is a k-dimensional tensor.
For a tensor $T$, its Frobenious norm, $\|T\|_F$,  is defined as the square root of the sum of the square of all the entries.  
...
if we want to emphasize the dependence on $s$ then we will use $\nu_t^{(s)}$.
\if0
...
Moreover, we assume there exists a constant $L$, such that $\| \E_{Y\sim \nu_t^{(s_i)}} [Y] \|_F\le L/\sqrt{t}$, $\| \E_{Y\sim \nu_t^{(\epsilon)}} [Y] \|_F \le L/\sqrt{t}$, and $\| \E_{Y\sim \nu_t^{(\epsilon)}} [Y^{\otimes4}] - 3(\E_{Y\sim \nu_t^{(\epsilon)}} [Y^{\otimes2}])^2 \|_F\le L/\sqrt{t}$.
...
Moreover, we assume the source signal function and the noise function are 'independent' in the sense that for positive $i,j$ that $i+j \le 4$,  
$\vert \E_{X\sim \nu_t^{(s)}} [X^{\otimes i}] \E_{Y\sim \nu_t^{(\epsilon)}} [Y^{\otimes j}]  - \E_{(X, Y)\sim \nu_t^{(s, \epsilon)}} [X^{\otimes i}\otimes Y^{\otimes j}]  \vert \le L/\sqrt{t}$, where $(s,\epsilon)$ is the function by concatenating $s$ and $\epsilon$ together.  
Note that the above assumptions does not lose any generality to the traditional stochastic setting of the ICA model with Gaussian noise. 
...
\[
\| \nabla^2f_{\nu_T^{(x)}}(\eta) - \nabla^2f_{\nu_T^{(As)}}(\eta) \|_2 \le \frac{\text{Poly}}{\sqrt{T}}, 
\] 
where $\text{Poly}$ is polynomial in $\{L_\eta, C, \sigma_{\max}(A), L\}$.
\end{prop}  
Denote $\frac{\text{Poly}}{\sqrt{T}}$ by $P$. Note that this error caused by noise will decay in a rate of $\sqrt{t}$.
...
Given a large enough $T$ (polynomial in $\{L_u, C, \sigma_{\max}(A), L, \ell^2\}$), assume that there exist a product measure $\mu$ such that $\xi$ is small enough polynomial in $\{\gamma_A, 1/d, 1/L_u, 1/\sigma_{\max}(A), \sigma_{\min}(A), 1/\kappa_{\max}, \kappa_{\min}, \ell\}$. 
\if0
Let 
 \begin{align*}
...
\right) \\
& \quad \times \left(d^5 A_{(2,\max)}^2A_{\max}^2\xi + P\right)
 \end{align*}
 ...
 \[
  ...
  \frac{1}{\gamma_A} \text{Poly}
\]
where $\widehat{A}$ is the output of the HKICA algorithm, and Poly is a polynomial in $\{C, L, d, L_u, \sigma_{\max}(A), 1/\sigma_{\min}(A), \kappa_{\max}, 1/\kappa_{\min}, 1/\ell\}$.
 \end{thm}
 \if0
\begin{remark}
