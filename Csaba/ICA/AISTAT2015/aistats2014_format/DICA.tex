...
\section{Analysis of the HK Algorithm}
...
We analyze the performance of the HK algorithm in this section.

\subsection{the HK algorithm}
\label{subsec:HKalg}
...
\end{algorithm}
\begin{remark}
The difficult in analyzing $\gamma$ of the HK algorithm is due to its dependence on the unknown mixing matrix $A$. 
...
In particular, in order for $\{v^{\top}A_1, \ldots, v^{\top}A_d \}$ to be independent for some direction $v$ from a Gaussian distribution $N(0,V)$(so that $\gamma$ could be easier to analyze), we would like to have $V = A^{-\top}A^{-1}$. 
