...
In a probabilistic setting, the concentration inequality holds in general between the empirical distribution and the true distribution (under some boundness conditions). 
However, in the deterministic setting such property is not guaranteed. Hence we need some assumptions that the given sequence $g$ does not go arbitrarily wild.
\begin{definition}
...
We will assume that the given sequence is close to some independent distribution in the measure of '***' distance. The result of \cite{arora2012provable} is stated in a deterministic setting, as follows. Denote the maximal singular value of a matrix $A$ by $\sigma_{\max}(A)$. Also, let $A_{(2,\min)} = \min_{i} \|A_i\|_2$ and $A_{(2,\max)} = \max_{i} \|A_i\|_2$.
\begin{thm}
There exists a constant $c>0$, such that for any zero mean product probability $\nu^*$, non-singular matrix $A$, and signals $g(t)$, if 
\[
D(\nu_T^{A^{-1}g}, \nu^*) \le c\frac{d_{\min}^{19/2}\kappa_{\min}^{19/2}\sigma_{\min}^{1/2}(A)}{d^{16}A_{(2,\max)}A_{\max}d_{max}^6\kappa_{\max}^6}
\]
...
\]
where $D_A(u) = \text{diag}\left(12(u^{\top}A_1)^2,\cdots, 12(u^{\top}A_d)^2\right)$. 
...
Thus given $D_4(\nu^{A^{-1}g},\nu^*) \le \frac{1}{2}\frac{d_{\min}\sigma_{\min}^2(A)\kappa_{\min}}{d^5A_{\max}^2 A_{(2,\max)}^2}$, 
every singular value of $\hat{B}^{-1}B$ is between $\frac{1}{\sqrt{1+\xi}}$ and $\frac{1}{\sqrt{1-\xi}}$. Thus there exists a orthonormal matrix $R^*$ such that 
\[
\|\hat{B}^{-1}B - R^*\|_2 \le \sqrt{\frac{\xi}{1-\xi}}.
\]

Consider the function 
\[
f_{B^{-1}A}(u) = \sum_i \kappa_i (u^{\top}B^{-1}A_i)^4 = \sum_i \kappa^{-1}_i d_i^{-2} (u^{\top}R_i)^4.
\]
Similarly we still use the empirical estimation of this function in the algorithm, 
\[
\left|\hat{f}_{\hat{B}^{-1}A}(u) - f_{B^{-1}A}(u) \right| \le \left|\hat{f}_{\hat{B}^{-1}A}(u) - f_{\hat{B}^{-1}A}(u) \right| + \left|f_{\hat{B}^{-1}A}(u) - f_{B^{-1}A}(u) \right|.
\]

Then for the second term 
\begin{align*}
& \left|f_{\hat{B}^{-1}A}(u) - f_{B^{-1}A}(u) \right| = \left| \sum_i \kappa_i(u^{\top}\hat{B}^{-1}A_i)^4 - \sum_i \kappa_i(u^{\top}B^{-1}A_i)^4\right| \\ 
& \quad = \left| \sum_i \kappa_i (u^{\top}\hat{B}^{-1}A_i - u^{\top}B^{-1}A_i)\left[(u^{\top}\hat{B}^{-1}A_i+u^{\top}B^{-1}A_i)\left((u^{\top}\hat{B}^{-1}A_i)^2+(u^{\top}B^{-1}A_i)^2\right)\right]\right| \\
& \quad \le \sum_i \kappa_i \left| u^{\top}\hat{B}^{-1}A_i - u^{\top}B^{-1}A_i \right| \left| u^{\top}\hat{B}^{-1}A_i+u^{\top}B^{-1}A_i\right| \left|(u^{\top}\hat{B}^{-1}A_i)^2+(u^{\top}B^{-1}A_i)^2 \right|
\end{align*}
Note that $\left| u^{\top} \hat{B}^{-1}A_i\right| \le \|u^{\top}\hat{B}^{-1}B\|_2 \le \frac{1}{\sqrt{1-\xi}}$ and $ \left| u^{\top} B^{-1}A_i\right| \le 1$.
\begin{align*}
& \left|f_{\hat{B}^{-1}A}(u) - f_{B^{-1}A}(u) \right| \le \frac{4}{(1-\xi)^{3/2}}\sum_i  \kappa_i\left| u^{\top}\hat{B}^{-1}A_i - u^{\top}B^{-1}A_i \right|  \\
& \quad \le \frac{4}{(1-\xi)^{3/2}} \|u^{\top}\hat{B}^{-1}AK - u^{\top}B^{-1}AK\|_2 \\
& \quad \le 8\sqrt{2}\|\hat{B}^{-1}B - I\|_2\|B^{-1}AK\|_2 \\
& \quad \le \frac{8\sqrt{2}\kappa^{1/2}_{\max}}{d_{\min}^{1/2}} \sqrt{\xi}
\end{align*}
...
Note that $f_{\hat{B}^{-1}A}(u) = \sum_i \kappa_i (u^{\top}\hat{B}^{-1}A_i)^4 $. Let $\alpha_i = |u^{\top}\hat{B}^{-1}A_i| \le \|\hat{B}^{-1}A_i\|_2 \le \frac{1}{\sqrt{(1-\xi)\kappa_{\min}d_{\min}}}$.
Thus, 
\[
\left|\hat{f}_{\hat{B}^{-1}A}(u) - f_{\hat{B}^{-1}A}(u) \right| \le \frac{16d^4}{((1-\xi)\kappa_{\min}d_{\min})^2}(D_4(\nu^{A^{-1}g},\nu^*)+D_2^2(\nu^{A^{-1}g},\nu^*))
\]

Thus, 
\[
\left|\hat{f}_{\hat{B}^{-1}A}(u) - f_{B^{-1}A}(u) \right| \le \frac{8\sqrt{2}d^{5/2}\kappa^{1/2}_{\max}A_{\max} A_{(2,\max)}}{d_{\min}\kappa^{1/2}_{\min}\sigma_{\min}(A)} \sqrt{D_4(\nu^{A^{-1}g},\nu^*)} + \frac{64d^4}{(\kappa_{\min}d_{\min})^2}(D_4(\nu^{A^{-1}g},\nu^*)+D_2^2(\nu^{A^{-1}g},\nu^*))
\]


Given the upper bound for $\left|\hat{f}_{\hat{B}^{-1}A}(u) - f_{B^{-1}A}(u) \right| $, we now need to pick parameters for the algorithm. To guarantee the interval for $\beta$ is not empty, $D_4(\nu^{A^{-1}g},\nu^*)$ has to be small enough. In general, we need to pick 6 parameters. It is easy to verify that the following scheme is valid for the algorithm.
\begin{align*}
 & \beta' = \frac{d_{\min}}{Qdd_{\max}}; \quad \delta' = \frac{d_{\min}\kappa_{\min}\beta'^2}{100d}; \quad  \gamma' = 3\sqrt{d}\beta'; \\
 & \beta = \frac{1}{K}\beta'; \quad \delta = \frac{d_{\min}\kappa_{\min}\beta^2}{100d}; \quad \gamma = 3\sqrt{d}\beta, 
\end{align*}
where $Q\ge 1200$ and $K\ge \frac{90000 d^{5/2}d_{\max}\kappa_{\max}}{d_{\min}\kappa_{\min}\beta'}$ will be specified later. 
