...
Let 
 ...
 \]
 and $\delta = \gamma -  2\frac{\sigma_{\max}(A)}{\sigma_{\min}(A)}Q$. 
 Assume the following conditions hold:
 \begin{itemize}
 \item $\widehat{M}$ has distinct eigenvalues;
 \item $\gamma > 4\frac{\sigma_{\max}(A)}{\sigma_{\min}(A)} Q$
 \item $\min_{i,j:i\neq j} \|A_i - A_j\|_2 > \frac{4}{\delta}\frac{\sigma_{\max}^2(A)}{\sigma_{\min}(A) } Q$;
 \item $\xi \le \min\{1,A^2_{(2,\min)}\} \frac{\tilde{\kappa}_{\min}\sigma_{\min}^2(A)}{d^5 A_{(2,\max)}^2A_{\max}^2}$.
 \end{itemize}
...
It remains to analyze the parameter $\gamma$. This parameter could be bounded from below with constant probability, as follows. We denote this event by $E_{\gamma}$.
\begin{thm}
...
\begin{remark}
Even Theorem \ref{thm:gammabound} provides a probabilistic lower bound for $\gamma$, such lower bound is still vague. 
...
Basically both conditions will be satisfied when $D_4(\nu,\nu_t)$ and $D_2(\nu,\nu_t)$ are small enough, 
which happens with high probability in a probabilistic setting.

In summary, the efficiency of the algorithm of \citet{DHsu2012} depends on $\min_{i\neq j}\|A_i-A_j\|_2$ which would be difficult to analyze.
...
Note that it can be bounded from lower by $\frac{4}{\sqrt{ed}(d^2+d)}$ with probability at least $1/2$. The proof is similar to the proof of theorem \ref{thm:gammabound}.
\end{remark}
...
$\sigma_{\min}(\nabla^2f(\psi)) \ge \tilde{\kappa}_{\min}\sigma_{\min}^2(A)$.
\end{lemma}
\begin{proof}
For any unit vector $v$, $v^{\top}AKA^{\top}v \ge \tilde{\kappa}_{\min} \|v^{\top}A\|_2^2 \ge \tilde{\kappa}_{\min}\sigma_{\min}^2(A)$.
\end{proof}

Lastly, we still need to bound $\|M - \widehat{M}\|_2$.
\begin{lemma}
\label{lem:Mvariation_alg}
Given that $\xi \le \min\{1,A^2_{(2,\min)}\} \frac{\tilde{\kappa}_{\min}\sigma_{\min}^2(A)}{d^5 A_{(2,\max)}^2A_{\max}^2}$, 
\[ 
...
Given that $\xi \le \min\{1,A^2_{(2,\min)}\} \frac{\tilde{\kappa}_{\min}\sigma_{\min}^2(A)}{d^5 A_{(2,\max)}^2A_{\max}^2}$, apply Lemma \ref{lem:Mvariation} and \cref{lem:nablavariation},
\begin{align*}
