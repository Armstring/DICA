...
Note that $M$ and $\widehat{M}$ are not symmetric. Thus in practice for the algorithm's output to be meaningful, we need $\widehat{M}$ to still have $d$ real eigenvalues.
Intuitively, the stability of this property is also close related to the quantity $\gamma$. 

Note that $\gamma$ is invariant to the norm of $A_i$, and all the $\psi^{\top}A_i$s are not 0. Thus, we can rescale $A_i$, such that $|\psi^{\top}A_i| = 1$. Now that $\gamma = |(\phi^{\top}A_i)^2 - (\phi^{\top}A_j)^2|$, this quantity could be bounded from below with constant probability, as follows. We denote this event by $E_{\gamma}$.
\begin{thm}
...
On one hand, even Theorem \ref{thm:gammabound} provides a probabilistic lower bound for $\gamma$, such lower bound is still vague since that each column of $A$ has been rescaled with different factors, which makes $\min_{i\neq j}\|A_i-A_j\|_2$ unpredictable. To investigate how much each $A_i$ is rescaled, we are interested in the behavior of $\min_i |\psi^{\top}A_i|$.
\begin{lemma}
\label{lem:dmin}
...
\STATE Compute $\widehat{T}_1 = \frac{1}{12}\nabla^2 (\widehat{f}(\widehat{B}^{-\top}\phi_1))$;
\STATE Sample $\phi_2$ from the unit sphere of $\real^d$;
\STATE Compute $\widehat{T}_2 = \frac{1}{12}\nabla^2 (\widehat{f}(\widehat{B}^{-\top}\phi_2))$;

\STATE Compute all the eigenvectors of $\widehat{T}_1\left(\widehat{T}_2\right)^{-1}$, $\widehat{R} = \{\mu_1,\ldots,\mu_d\}$;
...
\end{equation}
where $\Lambda_i = \text{diag}\left((\phi_i^{\top}R^*R_1)^2, \cdots, (\phi_i^{\top}R^*R_d)^2\right)$, and $T = T_1T_2^{-1}$. 
