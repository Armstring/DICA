...
\end{remark}
Denote the maximal singular value of a matrix $A$ by $\sigma_{\max}(A)$. Also, let $A_{(2,\min)} = \min_{i} \|A_i\|_2$, $A_{(2,\max)} = \max_{i} \|A_i\|_2$, and $A_{\max} = \max_{i,j} |A_{i,j}|$.
...
Note that sample a vector $\psi$ from a unit sphere, with probability~1, $\min_i |\psi^{\top}A_i|\neq 0$.  
We are interested in the behave of $\min_i |\psi^{\top}A_i|$.
\begin{lemma}
\label{lem:dmin}
With probability at least $\exp\{-\frac{1}{\sqrt{\pi}}\}$, $\min_i |\psi^{\top}A_i| \ge \frac{12}{d}A_{(2,\min)} \ge \frac{12\min_i\|A_i\|_2}{d}$.
\end{lemma}
Since we can only recover $A$ up to some scale, we can replace our goal by recovering the scaled $A$ such that $\psi^{\top}A = 1$. 
Moreover, by Lemma \ref{lem:dmin}, the scaled $s_i$ will still be bounded by $\frac{dC}{12\min_i\|A_i\|_2}$ with probability at least $\exp\{-\frac{1}{\sqrt{\pi}}\}$. 
The remainder of this section is based on the event that $\psi^{\top}A = 1$ and $s_i$ will still be bounded by $\frac{dC}{12\min_i\|A_i\|_2}$, denoted by $\tilde{C}$.
\citep{DHsu2012}'s algorithm is also modified for such event. 
...
If non-singular matrix $\widehat{X} = X+E$ satisfying that $\sigma_{\min}(X)>\|E\|_2$, then  $\|\widehat{X}^{-1} - X^{-1}\|_2 \le \|X\|_2^2\|E\|_2 + \|X\|_2\|E\|_2^2$.
\end{lemma}
\begin{proof}
Note that 
\[
...
= & \frac{\|E_1\|_2}{\sigma_{\min}(Y)} + \|X\|_2\|Y\|_2^2\|E_2\|_2 + O(\max\{\|E_1\|_2^2 , \|E_2\|_2^2\}).
\end{align*}
\end{proof}
...
\[
\|\nabla^2 f(\eta) - \nabla^2 \widehat{f}(\eta)  \|_2 \le \|\eta\|_2^2 \xi,
\]
where $\xi = d^5 A_{(2,\max)}^2A_{\max}^2\left( 4\tilde{C}^2D_2(\nu, \nu_t) + D_4(\nu, \nu_t)\right)$.
\end{lemma}
\begin{proof}
...
\begin{proof}
Let $E_1 = \nabla^2 f(\phi) - \nabla^2 \widehat{f}(\phi)$ and $ E_2 = \nabla^2 f(\psi) - \nabla^2 \widehat{f}(\psi)$. Then $\|E_1\|_2 , \|E_2\|_2 \le \xi$.
\begin{align*}
\|M - \widehat{M}\|_2 =\, & \|(\nabla^2 f(\phi))(\nabla^2f(\psi))^{-1} - (\nabla^2 \widehat{f}(\phi))(\nabla^2\widehat{f}(\psi))^{-1} \|_2 \\
\le \, & \left(\frac{1}{\sigma_{\min}\left(\nabla^2f(\psi)\right)} +  \|\nabla^2 f(\phi)\|_2 \|\nabla^2 f(\psi)\|_2^2\right)\xi+O(\xi^2).
\end{align*}
Note that $\nabla^2f(\psi) = AKA^{\top}$ and $\nabla^2f(\phi) = AK\Lambda A^{\top}$ where $\Lambda = \text{diag}\left((\phi^{\top}A_1)^2,\cdots, (\phi^{\top}A_d)^2\right)$.
Thus, 
\[ 
\|M - \widehat{M}\|_2 \le \left( \frac{1}{\sigma_{\min}^2(A)\kappa_{\min}}+ \sigma_{\max}^6(A)\kappa_{\max}^3A_{(2,\max)}^2\right) \xi + O(\xi^2).
\]
\end{proof}
...
\begin{remark}
When will the conditions of this theorem be satisfied?
\end{remark}
...
\] 
Note that $\nabla^2f(\psi) = \sum_{i=1}^{d} \kappa_iA_iA_i^{\top} = AKA^{\top}$. Thus, $\sigma_{\min}(\nabla^2f(\psi)) = \min_{v:\|v\|_2=1}\|\sum_{i=1}^{d} \kappa_i(\psi^{\top}A_i)^2A_iA_i^{\top}v\|_2$ 

\begin{claim}
$\sigma_{\min}(\nabla^2f(\psi)) \ge \kappa_{\min}\min_i\|A_i\|_2^2$.
\end{claim}
\begin{proof}
For any unit vector $v$, $v^{\top}AKA^{\top}v \ge \kappa_{\min} \|v^{\top}A\|_2^2 \ge \kappa_{\min}\min_i\|A_i\|_2^2$.
\end{proof}
The next
\begin{thm}
...
& \ge 2 \min_t |\phi^{\top}A_t|\frac{\min_{s\neq t} \|A_s-A_t\|_2}{\sqrt{ed}(d^2+d)},
\end{align*}
