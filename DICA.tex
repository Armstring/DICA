...
We analyze \citet{DHsu2012}'s algorithm in a deterministic setting, so that we can decouple the dependences of the learning error caused by the sampling procedure of $\phi$ and $\psi$, and that of the finite-sample estimation.
...
\begin{definition}
Given two distributions $F_1$ and $F_2$ in $\real^d$, $D_k(F_1,F_2) = \sup_{f\in\mathcal{F}} |\int f(x)dF_1(x) - \int f(x)dF_2(x)|$, where $\mathcal{F}$ is the set of all monomials up to degree $k$.
\end{definition}  
...
\end{lemma}
Denote this event by $\Epsi$. The performance of \citet{DHsu2012}'s algorithm depends on this parameter, as shown in the following theorem.

 \begin{thm}
 \label{thm:efficiency}
Let 
 \[ 
 Q= \frac{4d^7A_{(2,\max)}^4A_{\max}^2\kappa_{\max}\sigma_{\max}^2(A) + 2\sqrt{2}\sqrt{\pi}d^6A_{(2,\max)}^2A_{\max}^2\kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)}{\pi\kappa^2_{\min}A^4_{(2,\min)}\sigma_{\min}^4(A)} \xi.
 \] 
...
The behavior of the parameter $\gamma$ plays an important role in the analysis of \cite{DHsu2012}'s algorithm. 
In fact, $\gamma$ affects not only the theoretical upper bound in \ref{thm:efficiency}, but also the validity its conditions.

Note that $\gamma$ is invariant to the norm of $A_i$, and all the $\psi^{\top}A_i$s are not 0. 
Thus, we can rescale $A_i$, such that $|\psi^{\top}A_i| = 1$. 


Now that $\gamma = |(\phi^{\top}A_i)^2 - (\phi^{\top}A_j)^2|$, this quantity could be bounded from below with constant probability, as follows. We denote this event by $E_{\gamma}$.
\begin{thm}
\label{thm:gammabound}
With probability at least $1/8$,
\[
\gamma \ge \frac{\sqrt{2\pi}A_{(2,\min)}\min_{i\neq j}\|A_i-A_j\|_2}{\sqrt{ed}(d^3+d^2)}.
\]
\end{thm}
\begin{proof}
For any $i$ and $j$,
...
Thus on the event $\Epsi$, with probability at least $1/2$,
\[
\gamma \ge \frac{\sqrt{2\pi}A_{(2,\min)}\min_{i\neq j}\|A_i-A_j\|_2}{\sqrt{ed}(d^3+d^2)}.
\]
\end{proof}

On one hand, even Theorem \ref{thm:gammabound} provides a probabilistic lower bound for $\gamma$, such lower bound is still vague.
Note that each column of $A$ has been rescaled with different factors, which makes $\min_{i\neq j}\|A_i-A_j\|_2$ unpredictable.

On the other hand, note that $A$ can only be recovered up to scaling, we can instead start from the assumption that $\psi^{\top}A = 1^{\top}$. 
...
First note that $\min_{i,j:i\neq j} \|A_i - A_j\|_2 > \frac{8}{\gamma}\frac{\sigma_{\max}^2(A)}{\sigma_{\min}(A) } Q$ actually implies  $\gamma > 4\frac{\sigma_{\max}(A)}{\sigma_{\min}(A)} Q$, 
by the fact that $\sigma_{\max}(A) \ge \frac{1}{\sqrt{2}} \min_{i,j:i\neq j} \|A_i - A_j\|_2$. 
Moreover, the second condition (or both) will be satisfied when the event in Theorem \ref{thm:gammabound} holds, and 
\[
\min_{i,j:i\neq j} \|A_i - A_j\|_2 >\left( \frac{8\sqrt{ed}(d^3+d^2)\sigma_{\max}^2(A)}{\sqrt{2\pi}A_{(2,\min)}\sigma_{\min}(A) } Q\right)^{1/2}.
\]
This inequality is still related to $\psi$ and the rescaled $A$, which makes the probability of its validity vague.


We summarize the results above, as the following theorem.
\begin{thm}
...
Let 
 ...
 \begin{enumerate}
 \item $\widehat{M}$ has distinct eigenvalues;
 \item $\psi^{\top}A_i = 1$ for $1\le i \le d$;
 \item $\min_{i,j:i\neq j} \|A_i - A_j\|_2 >\left( \frac{8\sqrt{ed}(d^3+d^2)\sigma_{\max}^2(A)}{\sqrt{2\pi}A_{(2,\min)}\sigma_{\min}(A) } Q\right)^{1/2}$;
 \item $\xi \le \frac12\frac{\kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)}{d^5 A_{(2,\max)}^2A_{\max}^2}$.
  \end{enumerate}
 Then there exist a permutation $\pi$ and constants $\{c_1,\ldots,c_d\}$, such that with probability at least $1/8$,
 \[
  \max_{1\le k\le d}\| c_1\widehat{A}_{\pi(k)} - A_k\|_2 \le \frac{4\sqrt{ed}(d^3+d^2)}{\sqrt{2\pi}\min_{i\neq j}\|A_i-A_j\|_2} \frac{\sigma_{\max}^2(A)}{A_{(2,\min)}\sigma_{\min}(A)}Q .
  \]
  and therefore,
 \[
 \sum_{k=1}^{d}\| c_1\widehat{A}_{\pi(k)} - A_k\|_2 \le \frac{4d^3\sqrt{ed}(d+1)}{\sqrt{2\pi}\min_{i\neq j}\|A_i-A_j\|_2} \frac{\sigma_{\max}^2(A)}{A_{(2,\min)}\sigma_{\min}(A)}Q .
 \]
 \end{thm}
 
In summary, the efficiency of the algorithm of \citet{DHsu2012} depends on $\min_{i\neq j}\|A_i-A_j\|_2$ which could be difficult to analyze (since $A$ has been rescaled).
...
To avoid this problem, we modify \citet{DHsu2012}'s algorithm based on an idea from the paper \citet{arora2012provable}, so that the performance of the new algorithm doesn't depend on $\min_{i\neq j}\|A_i-A_j\|_2$ or $\gamma$.


\subsubsection{Modified \citet{DHsu2012}'s algorithm}
Note that $M$ and $\widehat{M}$ are not symmetric. Thus in practice the algorithm could return complex result which is meaningless, if $\widehat{M}$ does not have $d$ real eigenvalues. 
Intuitively, the chance of such $\widehat{M}$ happens will also depend on $\gamma$ (the minimal gaps between eigenvalues). 
We modify \citet{DHsu2012}'s algorithm to avoid this problem in this section.

Assume $B$ is a matrix such that $\nabla^2 (f(\psi)) = AKD_{\psi}A^{\top}\stackrel{\Delta}{=:}BB^{\top}$. 
So there exists an orthonormal matrix $R$ such that $B =AK^{1/2}D_{\psi}^{1/2}R^{\top}$, where $D_{\psi} = \text{diag}\left((\psi^{\top}A_1)^2,\cdots, (\psi^{\top}A_d)^2\right)$.
Then we define $M$ in a new way:
\[
M = B^{-1}\nabla^2f(\phi)B^{-\top} = R
\left(
\begin{array}{ccc}
...
R^{\top}.
\]
Once $R$ is known, $BR = AK^{1/2}D_\phi^{1/2}$ is the matrix we are interested.
Thus the modified algorithm is as follows.
\begin{algorithm}[H]
\caption{Modified  \citet{DHsu2012}'s algorithm \label{alg:icaModHsu}}
\begin{algorithmic}[1]
\INPUT $g(k)$ for $1\le k \le t$. 
\OUTPUT An estimation of the mixing matrix $A$. 
\STATE Sample $\psi$ from the unit sphere of $\real^d$;
\STATE Evaluate $\nabla^2\widehat{f}(\psi)$, \\
\quad where $\widehat{m_p}(\eta) = \frac{1}{t}\sum_{k=1}^{t} (\eta^{\top}g(k))^p$, and $\widehat{f}(\eta) = \frac{1}{12}\big(\widehat{m_4}(\eta) - 3\widehat{m_2}(\eta)^2 \big)$;
\STATE Compute $\widehat{B}$ such that $\nabla^2\widehat{f}(\psi) = \widehat{B}\widehat{B}^{\top}$;
\STATE Sample $\phi$ from the unit sphere of $\real^d$;
\STATE Compute $\widehat{M} = \widehat{B}^{-1}(\nabla^2\widehat{f}(\phi))\widehat{B}^{-\top}$;
\STATE Compute all the eigenvectors of $\widehat{M}$, $\widehat{R} = \{\mu_1,\ldots,\mu_d\}$;
\STATE Return $\widehat{A} = \widehat{B}\widehat{R}$.
\end{algorithmic}
\end{algorithm}
Note that $\widehat{M}$ is always symmetric, so it solves the complex output problem of \citet{DHsu2012}'s algorithm. However, the modified version still requires an eigen-decompostion of a matrix whose eigenvalues are $\lambda_i$'s. 
Thus, we will still have a theoretical result depending on $\gamma$.


\subsection{Determined ICA}
In this subsection we introduce a new algorithm whose performance does not depends on $\min_{i\neq j}\|A_i-A_j\|_2$ or $\gamma$. The idea is inspired by the paper of \citet{arora2012provable} and the one of \cite{frieze1996learning}.
...
\]
\end{thm}
\subsubsection{Behavior of $\gamma_R$}
Even though the behavior of $\gamma$ is unknown, we can analyze the behavior of $\gamma_R$. 
...
\]
Also note that $ \left| \int (\eta^{\top}Ax)^2\,d\nu(x) \right| \le d^2A_{(2,\max)}^2 C^2$, and
$\left| \int A_ixx^{\top}A_j^{\top} \,d\nu_t(x) \right| \le d^2A_{\max}^2 C^2$.
...
\le\, & 2 d^4  A_{(2,\max)}^2A_{\max}^2C^2D_2(\nu, \nu_t).
\end{align*}
Similarly,
\[
\left| (G_3)_{i,j} - (\widehat{G_3})_{i,j} \right| \le 2 d^4  A_{(2,\max)}^2A_{\max}^2C^2D_2(\nu, \nu_t).
\]
Thus for any $1\le i,j\le d$,
\begin{align*}
\left|\left(\nabla^2 f(\eta)\right)_{i,j} - \left(\nabla^2 \widehat{f}(\eta) \right)_{i,j} \right| 
\le 
d^4  A_{(2,\max)}^2A_{\max}^2\left( 6C^2D_2(\nu, \nu_t) + D_4(\nu, \nu_t)\right).
\end{align*}
Therefore, 
\[
\|\nabla^2 f(\eta) - \nabla^2 \widehat{f}(\eta)  \|_2 \le \|\nabla^2 f(\eta) - \nabla^2 \widehat{f}(\eta)  \|_F \le d^5  A_{(2,\max)}^2A_{\max}^2\left( 6C^2D_2(\nu, \nu_t) + D_4(\nu, \nu_t)\right).
\]
\end{proof}
...
\[ 
\|M - \widehat{M}\|_2 \le \frac{4d^7A_{(2,\max)}^4A_{\max}^2\kappa_{\max}\sigma_{\max}^2(A) + 2\sqrt{2}\sqrt{\pi}d^6A_{(2,\max)}^2A_{\max}^2\kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)}{\pi\kappa^2_{\min}A^4_{(2,\min)}\sigma_{\min}^4(A)} \xi.
\]
\end{lemma}
\begin{proof}
...
Given that $\xi \le  \frac{\sqrt{2\pi}\kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)}{4d^6 A_{(2,\max)}^2A_{\max}^2}$, the condition in Lemma \ref{lem:Mvariation} holds. 
Then apply Lemma \ref{lem:Mvariation} and \cref{lem:nablavariation}, we have
\begin{align*}
...
\begin{proof}[{\bf Proof of Theorem  \ref{thm:efficiency}}]
 By \cref{lem:eigenvectorvariation}, 
  \[
  \max_{1\le k\le d}\| c_1\widehat{A}_{\pi(k)} - A_k\|_2 \le 4 \frac{\sigma_{\max}^2(A)}{\gamma \sigma_{\min}(A)}\|M - \widehat{M} \|_2, 
  \]
 and 
 \[
...
\[
\| c_k\widehat{R}_{\pi(k)} - (R^*R)_k\|_2 \le \frac{4}{\gamma_R} \|E\|_2\,,
\]
...
\]
The rest of the proof is by Lemma \ref{lem:eigenvectorvariation}.
\end{proof}
It still remains to bound $\|E\|_2$. The following lemma shows the stability of the square root of matrix.
\begin{lemma}
...
Given that $\xi \le \frac{\sqrt{\pi}\kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)}{2\sqrt{2}d^6A_{(2,\max)}^2A_{\max}^2}$
(so $\bar{\xi} \le 1/2$), under the event $\Epsi$ there exists an orthonormal matrix $R^*$ such that 
\[
\|\widehat{B}^{-1}B - R^*\|_2 \le \bar{\xi}.
\]
\end{lemma}
\begin{proof}
By Lemma \ref{lem:matrixsquareroot}, every singular value of $\widehat{H}^{-1}H$ is bounded between $\frac{1}{\sqrt{1 + \|\left(\nabla^2f(\phi)\right)^{-1}\|_2 \|E\|_2}}$ and $\frac{1}{\sqrt{1 - \|\left(\nabla^2f(\phi)\right)^{-1}\|_2 \|E\|_2}}$. 
Note that by Lemma \ref{lem:boundsigmaminnabla},  under the event $\Epsi$, $\|\nabla^2f(\phi)\|_2 \ge \frac{\sqrt{2\pi}}{2d}\kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)$. Thus,
\[
\|\left(\nabla^2f(\phi)\right)^{-1}\|_2 \|E\|_2 \le \frac{\sqrt{2}d^6A_{(2,\max)}^2A_{\max}^2}{\sqrt{\pi}\kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)}\xi = \bar{\xi}.
\]
Also note that  given $\xi \le \frac{\sqrt{\pi}\kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)}{2\sqrt{2}d^6A_{(2,\max)}^2A_{\max}^2}$, $\bar{\xi} \le 1/2$. 
...
Moreover, $f(R^*B^{-\top}\phi) = \sum_i\kappa_i(\phi^{\top}R^*B^{-1}A)_i^4 = \sum_i \kappa_i(\phi^{\top}R^*RK^{-1/2}D_{\psi}^{-1/2})_i^4=\sum_i\kappa_i^{-1}D_{\psi,i}^{-2}(\phi^{\top}R^*R)_i^4$.
Define $T$ as follows. For $i = 1,2$, 
\begin{equation}
\label{eq:T}
T_i = \frac{1}{12}\nabla^2(f(R^*B^{-\top}\phi_i)) = R^*RK^{-1}D_{\psi}^{-2}\Lambda_i(R^*R)^{\top},
\end{equation}
where $\Lambda_i = \text{diag}\left((\phi_i^{\top}R^*R_1)^2, \cdots, (\phi_i^{\top}R^*R_d)^2\right)$. 
Then $T = R^*R \Lambda_1 \Lambda_2^{-1} (R^*R)^{\top} = R^*R \Lambda (R^*R)^{\top}$, where $\Lambda = \text{diag}\left((\frac{\phi_1^{\top}R^*R_1}{\phi_2^{\top}R^*R_1})^2, \cdots, (\frac{\phi_1^{\top}R^*R_d}{\phi_2^{\top}R^*R_d})^2\right)$. 

\begin{lemma}
\label{lem:Binversenablavariation}
Given that $\xi \le \frac{\sqrt{\pi}\kappa_{\min}A^2_{(2,\min)}\sigma_{\min}^2(A)}{2\sqrt{2}d^6A_{(2,\max)}^2A_{\max}^2}$
(so $\bar{\xi} \le 1/2$), then under the event $\Epsi$ for $\phi \in \{\phi_1, \phi_2\}$,
\[
\|\nabla^2(f(R^*B^{-\top}\phi)) - \nabla^2(\widehat{f}(\widehat{B}^{-\top}\phi))\|_2
\le \frac{4d^9}{(1-\bar{\xi})^2\pi^2\kappa_{\min}^2A^4_{(2,\min)}}\xi + 2\bar{\xi} 
\le\frac{16d^9}{\pi^2\kappa_{\min}^2A^4_{(2,\min)}}\xi + 2\bar{\xi} = \widehat{\xi}.
\]
\end{lemma}
\begin{proof}
Note that by Lemma \ref{lem:nablavariation} $\|E\|_2 = \|\nabla^2 f(\eta) - \nabla^2 \widehat{f}(\eta)\|_2 \le d^5  A_{(2,\max)}^2A_{\max}^2\xi$, and 
\[
\|\nabla^2(f(R^*B^{-\top}\phi)) - \nabla^2(\widehat{f}(\widehat{B}^{-\top}\phi))\|_2
\le 
...
\begin{itemize}
\item $|\phi^{\top}\widehat{B}^{-1}A_i| \le \|\widehat{B}^{-1}B\|_2\|B^{-1}A_i\|_2\le \frac{\sqrt{2}d}{\sqrt{1-\bar{\xi}}\sqrt{\pi}\kappa_{\min}^{1/2}A_{(2,\min)}}$;
...
\frac{4d^8}{(1-\bar{\xi})^2\pi^2\kappa_{\min}^2A^4_{(2,\min)}}D_4(\nu,\nu_t).
\]
Also note that for $1\le i, j\le d$ and any distribution $\Prob{y}$ of $y$,
\begin{itemize}
\item $|\int(\phi^{\top}\widehat{B}^{-1}y)(\widehat{B}^{-1})_{i:}y d\Prob{y}| \le \frac{2d^3C^2}{(1-\bar{\xi})\pi\kappa_{\min}A^2_{(2,\min)}}$; 
\item $|\int(\phi^{\top}\widehat{B}^{-1}y)^2 d\Prob{y}|\le \frac{2d^3C^2}{(1-\bar{\xi})\pi\kappa_{\min}A^2_{(2,\min)}}$;
\item $|\int(\widehat{B}^{-1})_{i:}yy^{\top}(\widehat{B}^{-\top})_{:j}d\Prob{y}|\le \frac{2d^3C^2}{(1-\bar{\xi}))\pi\kappa_{\min}A^2_{(2,\min)}}$.
\end{itemize}
Thus, similar to \ref{proofoflemma},
\[
|(G_2(\widehat{B}^{-\top}\phi))_{i,j} - (\widehat{G_2}(\widehat{B}^{-\top}\phi))_{i,j}| \le
\frac{4d^7C^2}{(1-\bar{\xi})^2\pi^2\kappa_{\min}^2A^4_{(2,\min)}}D_2(\nu,\nu_t),
\]
and 
\[
...
\le
\frac{4d^8}{(1-\bar{\xi})^2\pi^2\kappa_{\min}^2A^4_{(2,\min)}}\xi, 
\]
thus, 
\begin{equation}
\label{eq:fBhatfhatBhat}
 \|\nabla^2(f(\widehat{B}^{-\top}\phi)) - \nabla^2(\widehat{f}(\widehat{B}^{-\top}\phi))\|_2 \le 
 \frac{4d^9}{(1-\bar{\xi})^2\pi^2\kappa_{\min}^2A^4_{(2,\min)}}\xi.
\end{equation}
...
\|\nabla^2(f(B^{-\top}\phi)) - \nabla^2(\widehat{f}(\widehat{B}^{-\top}\phi))\|_2
\le 
\frac{4d^9}{(1-\bar{\xi})^2\pi^2\kappa_{\min}^2A^4_{(2,\min)}}\xi + 2\bar{\xi}.
\]
\end{proof}

\begin{lemma}
\label{lem:Tvariantion}
On the event $\Epsi$, $\Ephione$, and $\Ephitwo$, assume that $\widehat{\xi} \le \frac{\pi}{4d^2\kappa_{\max}A^4_{(2,\max)}}$, then 
\[
...
\begin{proof}
On the event $\Epsi$, $\Ephione$, and $\Ephitwo$,
\begin{align*}
& \|H\|_2 \le \|K^{-1/2}D_{\psi}^{-1}\Lambda_1^{1/2}\|_2 \le \frac{2d}{\sqrt{2\pi}\kappa_{\min}^{1/2}A^2_{(2,\min)}}; \\
& \|H^{-1}\|_2 = \|K^{1/2}D_{\psi}\Lambda_1^{-1/2}\|_2 \le \frac{2d\kappa_{\max}^{1/2}A^2_{(2,\max)}}{\sqrt{2\pi}}; \\
& \|T_1^{-1}\|_2 \le \frac{2d^2\kappa_{\max}A^4_{(2,\max)}}{\pi}; \quad \|T_2^{-1}\|_2 \le \frac{2d^2\kappa_{\max}A^4_{(2,\max)}}{\pi}; \\
& \sigma_{\min}(T_2) \ge \frac{\pi}{2d^2\kappa_{\max}A^4_{(2,\max)}}.
\end{align*}
Note that given $\widehat{\xi} \le \frac{\pi}{4d^2\kappa_{\max}A^4_{(2,\max)}}$, $\sigma_{\min}(T_2) \ge 2\|T_2 - \widehat{T}_2\|_2$, then by Lemma \ref{lem:inversevariation},
\[
\|T_2^{-1} - \widehat{T}_2^{-1}\|_2 \le \frac{2}{\sigma_{\min}^2(T_2)}\|T_2 - \widehat{T}_2\|_2 
\le \frac{8d^4\kappa_{\max}^2A^8_{(2,\max)}}{\pi^2}\widehat{\xi}.
\]
Also, 
\begin{align*}
\|\widehat{H}\|_2 \le & \|H\|_2\|H^{-1}\widehat{H}\|_2 \\
	\le & \|H\|_2\sqrt{1 + \|T_1^{-1}\|_2\|\widehat{T}_1 - T_1\|_2} \\
	\le & \frac{2d}{\sqrt{2\pi}\kappa_{\min}^{1/2}A^2_{(2,\min)}} \sqrt{1 + \frac{2d^2\kappa_{\max}A^4_{(2,\max)}}{\pi}\widehat{\xi}} \\
	\le & \frac{2d}{\sqrt{\pi}\kappa_{\min}^{1/2}A^2_{(2,\min)}}.
\end{align*}
Moreover, by Lemma \ref{lem:matrixsquareroot},
\[
\|I - H^{-1}\widehat{H}\|_2 \le \sqrt{\|T_1^{-1}\|_2\|T_1 - \widehat{T}_1\|_2} \le \sqrt{\frac{2d^2\kappa_{\max}A^4_{(2,\max)}}{\pi}\widehat{\xi}}.
\]
By triangular inequality,
\begin{align*}
\|T - \widehat{T}\|_2  = & \| H^{\top}T_2^{-1}H - \widehat{H}^{\top}\widehat{T}_2^{-1}\widehat{H}\|_2 \\
 \le & 	\| H^{\top}T_2^{-1}H - \widehat{H}^{\top}T_2^{-1}\widehat{H}\|_2  
 		+ \|\widehat{H}^{\top}T_2^{-1}\widehat{H} - \widehat{H}^{\top}\widehat{T}_2^{-1}\widehat{H}\|_2\\
 \le & 	\|\widehat{H}\|_2^2\|T_2^{-1} - \widehat{T}_2^{-1}\|_2 +
 		 \| H^{\top}T_2^{-1}H - \widehat{H}^{\top}T_2^{-1}H\|_2 +
 		 \| \widehat{H}^{\top}T_2^{-1}H - \widehat{H}^{\top}T_2^{-1}\widehat{H}\|_2 \\
 \le &  \|\widehat{H}\|_2^2\|T_2^{-1} - \widehat{T}_2^{-1}\|_2 + 
 		\|H\|_2^2\|T_2^{-1}\|_2\|I - H^{-1}\widehat{H}\|_2 + 
 		\|H\|_2\|\widehat{H}\|_2\|T_2^{-1}\|_2\|I - H^{-1}\widehat{H}\|_2 \\
 \le & \frac{32d^6\kappa_{\max}^2A^8_{(2,\max)}}{\pi^3\kappa_{\min}A^4_{(2,\min)}}\widehat{\xi} +
 		\frac{4\sqrt{2}d^5\kappa^{3/2}_{\max}A^6_{(2,\max)}}{\pi^{5/2}\kappa_{\min}A^4_{(2,\min)}} \sqrt{\widehat{\xi}} + 
 		\frac{8d^5\kappa^{3/2}_{\max}A^6_{(2,\max)}}{\pi^{5/2}\kappa_{\min}A^4_{(2,\min)}} \sqrt{\widehat{\xi}} \\
 \le & \frac{16d^5\kappa_{\max}^{3/2}A^6_{(2,\max)}}{\pi^{5/2}\kappa_{\min}A^4_{(2,\min)}}\sqrt{\widehat{\xi} }+
  		\frac{4\sqrt{2}d^5\kappa^{3/2}_{\max}A^6_{(2,\max)}}{\pi^{5/2}\kappa_{\min}A^4_{(2,\min)}} \sqrt{\widehat{\xi}} + 
  		\frac{8d^5\kappa^{3/2}_{\max}A^6_{(2,\max)}}{\pi^{5/2}\kappa_{\min}A^4_{(2,\min)}} \sqrt{\widehat{\xi}} \\
 \le & \frac{32\sqrt{2}d^5\kappa^{3/2}_{\max}A^6_{(2,\max)}}{\pi^{5/2}\kappa_{\min}A^4_{(2,\min)}} \sqrt{\widehat{\xi}} = Q.  		
\end{align*}
...
\le & A_{(2,\max)}\sigma_{\max}(A)\kappa_{\max}^{1/2}(1+\bar{\xi})^{1/2} \frac{4\sqrt{d}}{\gamma_R}\|\widehat{T} - T\|_2 + A_{(2,\max)}\sigma_{\max}(A)\kappa_{\max}^{1/2}(1+\bar{\xi})^{1/2}\bar{\xi}\\
\le & A_{(2,\max)}\sigma_{\max}(A)\kappa_{\max}^{1/2}(1+\bar{\xi})^{1/2} \left( \bar{\xi} + \frac{4\sqrt{d}}{\gamma_R}Q\right). \\
\end{align*}
Similarly, note that $\bar{\xi} \le 1/2$, thus
\[
