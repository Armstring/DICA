...
\section{Independence of Functions}
...
%Given two ergodic function $f$ and $g$, let $X_t$ (and $Y_t$ ) is the random variable induced by $\mu_t$ and $f$(and $g$, respectively). Then $\nu_{f+g,t}$ is the probability function of $X_t+Y_t$. Note that $X_t \xrightarrow{D} X$ and  $Y_t \xrightarrow{D} Y$.  \emph{However, $X_t+Y_t \xrightarrow{D} X+Y$ is not guaranteed!} 
...
When the domain of the function is discrete (e.g., the set of natural numbers),
	...
	is non-vacuous.
\begin{prop}
Assume that $f:\natural\rightarrow\real$ and $g:\natural\rightarrow\real$ are periodic functions with respective periods $T_1$ and $T_2$. 
...
If $f,g$ are independent ergodic functions then
$f+g$ is ergodic and
 $\nu^{(f+g)} = \nu^{(f)} * \nu^{(g)}$.
\end{prop}
\begin{proof}
...
By \cref{prop:comp} and the assumption that $f$ and $g$ are ergodic, it follows that $(f,g)$ is also ergodic.
...
\begin{thm}
\label{thm:CorofICA}
Let $f_1,\ldots,f_d$ be mutually independent real-valued, \todoc{We always have real-valued functions, right? So maybe mention this always or nowhere} ergodic functions and 
...
let $m_p(\eta) = \E_{x\sim \nu^{(s_i)}}[ (\eta^\top x)^p ]$ and let
\[
f(\eta) = \frac1{12} \left( m_4(\eta) - 3 m_2(\eta)^2 \right)\,.
\]

The algorithm of \citet{DHsu2012} is as follows. 
We denote $\widehat{E}$ as the empirical estimation of parameter $E$. 
\begin{algorithm}[H]
...
As shown in \citep{DHsu2012}, with probability one,
\begin{equation}
\label{eq:M}
M = A 
\left(
...
A^{-1},
\end{equation}
has distinct eigenvalues. Thus, if $M =(\nabla^2f(\phi))(\nabla^2f(\psi))^{-1} $ was used in \cref{alg:icaHsu} in place of $\widehat{M}$, the algorithm would return $A$ with probability one. 
Let 
\begin{equation}
\label{def:kappa}
\gamma =  \min_{i,j: i\neq j} \left\vert \left(\frac{\phi^{\top}A_i}{\psi^{\top}A_i}\right)^2 - \left(\frac{\phi^{\top}A_j}{\psi^{\top}A_j}\right)^2 \right\vert. 
\end{equation}
The following lemma shows a small perturbation of $M$ will only result in a small variation of its eigenvectors, under some conditions.

\begin{lemma}
\label{lem:eigenvectorvariation}
Denote $\widehat{M} = M+E$ be a perturbation of matrix $M$, where $M$ is defined in  \eqref{eq:M}. 
