...
%.Hence,  $\nu_t^{(f+g)} = \nu_t^{(f)}* \nu_t^{(g)}$. 
\todoc{Hmm, this only follows if $(X_{t,1},X_{t,2})$ are independent of each other, no? I don't think this proof will go through!}
which by \cref{cor:cw} will converge to the distribution of $X_1+X_2$. Note that $X_1$ and $X_2$ are independent, so their distribution is $\nu^{(f)}*\nu^{(g)}$. Thus,
\[
...
\todor[inline]{Yes...I guess just remove the convolution of $\nu_t$. By Theorem \ref{cor:cw}, $X_{t,1}+X_{t,2}$ converges to $X_1+X_2$. This is all we need.}
\if0
