...
\begin{lemma}
\label{lem:dmin}
With probability at least $\exp\{-\frac{1}{\sqrt{\pi}}\}$, $\min_i |\psi^{\top}A_i| \ge \frac{12}{d}A_{(2,\min)}^2 \ge \frac{12\min_i\|A_i\|_2}{d}$.
\end{lemma}
Since we can always at most recover $A$ up to some scale, we can replace our goal by recovering the scaled $A$ such that $\psi^{\top}A = 1$. 
...
\end{lemma}
\begin{proof}
Denote the $i$-th row of $A$ by $A_{:,i}$, and the $(i,j)$ entry of a matrix $M$ by $(M)_{i,j}$. Since $\|f_i\|_\infty \le \tilde{C}$,$\|f\|_2 \le \tilde{C}\sqrt{d}$. 
