...
\STATE Return $\widehat{A} = \widehat{B}\Psi$, where $\Psi$ is the matrix $(\mu_1,\ldots,\mu_d)$.
\end{algorithmic}
\end{algorithm}
...
Thus, $f(B^{-\top}\phi) = \sum_i\tilde{\kappa}_i(\phi^{\top}B^{-1}A)_i^4 = \sum_i \tilde{\kappa}_i(\phi^{\top}R\tilde{K}^{-1/2})_i^4=\sum_i\tilde{\kappa}_i^{-1}(\phi^{\top}R)_i^4$, and 
\[
\frac{1}{12}\nabla^2(f(B^{-\top}\phi)) = R\tilde{K}^{-1}DR^{\top},
\]
where $D = \text{diag}\left((\phi^{\top}R_1)^2, \cdots, (\phi^{\top}R_d)^2\right)$.

Note that by ********
\[
\|\frac{1}{12}\nabla^2(f(B^{-\top}\phi)) - \frac{1}{12}\nabla^2(\widehat{f}(\widehat{B}^{-\top}\phi))\|_2
\le ddd
\]
...
for $1\le i, j\le d$,
\begin{itemize}
\item $|\phi^{\top}\widehat{B}^{-1}A_i| \le \|\widehat{B}^{-1}\|_2A_{(2,\max)}\le \frac{A_{(2,\max)}}{\sqrt{1-\bar{\xi}}\sigma_{\min}(A)\tilde{\kappa}^{1/2}}$;
\item $|(\widehat{B}^{-1})_{i:}A_j| \le \frac{A_{(2,\max)}}{\sqrt{1-\bar{\xi}}\sigma_{\min}(A)\tilde{\kappa}^{1/2}}$;
\item d.
\end{itemize}
Thus for $1\le i, j\le d$,
\[
|(G_1(\widehat{B}^{-\top}\phi))_{i,j} - (\widehat{G_1}(\widehat{B}^{-\top}\phi))_{i,j}| \le
\frac{d^4A_{(2,\max)}^4}{(1-\bar{\xi})^2\sigma_{\min}^4(A)\tilde{\kappa}^{2}}D_4(\nu,\nu_t).
\]
Also note that for $1\le i, j\le d$ and any distribution $\Prob{y}$ of $y$,
\begin{itemize}
\item $|\int(\phi^{\top}\widehat{B}^{-1}y)(\widehat{B}^{-1})_{i:}y d\Prob{y}| \le d\|\widehat{B}^{-1}A\|_2^2\tilde{C}^2 \le $; 
\item $|\int(\phi^{\top}\widehat{B}^{-1}y)^2 d\Prob{y}|\le d\|\widehat{B}^{-1}A\|_2^2\tilde{C}^2$;
\item $|\int(\widehat{B}^{-1})_{i:}yy^{\top}(\widehat{B}^{-\top})_{:j}d\Prob{y}|\le d\|\widehat{B}^{-1}A\|_2^2\tilde{C}^2$.
\end{itemize}
Thus, 
\[
|(G_2(\widehat{B}^{-\top}\phi))_{i,j} - (\widehat{G_2}(\widehat{B}^{-\top}\phi))_{i,j}| \le
\frac{d^4A_{(2,\max)}^4}{(1-\bar{\xi})^2\sigma_{\min}^4(A)\tilde{\kappa}^{2}}D_2(\nu,\nu_t).
\]
