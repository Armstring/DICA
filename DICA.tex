...
By Lemma \ref{lem:dmin}, $\kappa_i$ is scaled by a factor of $ \frac{1}{A_{(2,\max)}} \le k_i \le \frac{2d}{\sqrt{2\pi}A_{(2,\min)}}$, denoted by $\tilde{\kappa}$. 
...
We analyze \citet{DHsu2012}'s algorithm in a deterministic setting, so that we can decouple the dependences of the learning error caused by the sample procedure of $\phi$ and $\psi$, and the finite-sample estimation.
...
\begin{remark}
Even Theorem \ref{thm:gammabound} provides a probabilistic lower bound for $\gamma$, such lower bound is still vague. On one hand, note that each column of $A$ has been rescaled with different factors, which makes it hard to bound $\min_{i\neq j}\|A_i-A_j\|_2$. On the other hand, if we begin with the assumption that $\psi^{\top}A = 1^{\top}$, then this quantity is in fact part of the setting of the problem.
\end{remark}
...
So the efficiency of the algorithm of \citet{DHsu2012} depends on $A_{(2,\min)}$ and $\min_{i\neq j}\|A_i-A_j\|_2$. We would expect that the performance is going to depends on $A_{(2,\min)}$. But what about $\min_{i\neq j}\|A_i-A_j\|_2$? This parameter $\gamma$ is introduce in the recovery of the eigenspace $A$. We introduce a new algorithm based on \citet{DHsu2012}'s one and the idea of \citet{arora2012provable} which shows that such dependence is not necessary. 

\subsection{Modified \citet{DHsu2012}'s algorithm}
Based on \citet{DHsu2012}'s algorithm, we proposed another algorithm which does not depends on $\min_{i\neq j}\|A_i-A_j\|_2$. This idea is inspired by the paper of \citet{arora2012provable}.
...
Given that $\xi \le \frac{\sigma_{\min}(A)\tilde{\kappa}_{\min}^{1/2}}{2d^5A_{(2,\max)}^2A_{\max}^2}$, there exist an orthonormal matrix $R^*$ such that 
\[
\|\widehat{B}^{-1}B - R^*\|_2 \le \bar{\xi},
\]
where $\bar{\xi} = \frac{d^5A_{(2,\max)}^2A_{\max}^2}{\sigma_{\min}(A)\tilde{\kappa}_{\min}^{1/2}} \xi$.
\end{lemma}
\begin{proof}
...
\[
\|\widehat{A}\Pi D(c) - A\|_2 \le 2\sigma_{\max}(A)\tilde{\kappa}_{\max}^{1/2} \left( \bar{\xi} + \frac{4\sqrt{d}}{\delta_R}\bar{\xi} + \frac{2\sqrt{d}d^5}{\delta_R(1-\bar{\xi})^2\tilde{\kappa}_{\min}^2}\xi\right). 
\]
\end{thm}
\begin{remark}
Note that the bound does not depends on $\gamma$ anymore but $\gamma_R$, and it can be bounded from lower by $\frac{4}{\sqrt{ed}(d^2+d)}$ with probability at least $1/2$. The proof is similar to the proof of theorem \ref{thm:gammabound}.
\end{remark}
...
We need to introduce two lemmas before we can prove the theorem. Note that $\nabla^2 (f(\psi)) = A\tilde{K}A^{\top}\stackrel{\Delta}{=:}BB^{\top}$. So there exists an
...
If $\gamma_R > 4 \|E\|_2$, and $\min_{i,j:i\neq j} \|R_i - R_j\|_2 > \frac{4}{\delta_R}\|E\|_2$, then there exist a permutation $\pi$ and constants $\{c_1,\ldots,c_d\}$, such that for $1\le k\le d$
\[
\| c_k\widehat{R}_{\pi(k)} - R_k\|_2 \le \frac{2}{\delta_R} \|E\|_2\,,
\]
...
\]
\end{proof}
\begin{proof}[Proof of Theorem \ref{thm:Modefficiency}]
...
\le  \frac{d^5}{(1-\bar{\xi})^2\tilde{\kappa}_{\min}^2}\xi + 2\bar{\xi}.
\]
Therefore, by Lemma \ref{lem:Teigenvectorvariation}, there exist a permutation $\pi$ and constants $\{c_1,\ldots,c_d\}$, such that for $1\le k\le d$
\[
...
& \le \sigma_{\max}(A)\tilde{\kappa}_{\max}^{1/2}(1+\bar{\xi})^{1/2} \|\widehat{T} - T\|_2 + \sigma_{\max}(A)\tilde{\kappa}_{\max}^{1/2}(1+\bar{\xi})^{1/2}\bar{\xi}\\
& \le  \sigma_{\max}(A)\tilde{\kappa}_{\max}^{1/2}(1+\bar{\xi})^{1/2} \left( \bar{\xi} + \frac{4\sqrt{d}}{\delta_R}\bar{\xi} + \frac{2\sqrt{d}d^5}{\delta_R(1-\bar{\xi})^2\tilde{\kappa}_{\min}^2}\xi\right) \\
