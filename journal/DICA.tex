...
\label{eq:HKICA-bound}
 ...
 \mathcal{C}(\mu)\min\left( \frac{1}{\gamma_A} (D_4(\nu^{(s)},\mu)+ \cQ(\nu^{(\eps)}) + D_4^{(d,d)}(\nu^{(As,\eps)})
  +N(\nu^{(\eps)}) + N( \nu^{(s)})), \Theta(\mu) \right)
\end{equation}
where $\hat{A}$ is the output of the HKICA algorithm, $\mathcal{C}(\mu)$ and $\Theta(\mu)$ are problem dependent constants, polynomial in $(\sigma_{\max}(A), 1/\sigma_{\min}(A), 1/\kappa_{\min}(\mu),1/\delta, d, L)$.
\end{theorem}
\begin{remark}
...
Note that the $\mathcal{C}(\mu)$ here may be different from that in Theorem \ref{thm:efficiency}.
